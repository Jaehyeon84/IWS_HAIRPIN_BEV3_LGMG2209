...
//        protected JSONReader m_clsJSON = new JSONReader();
        public JSONReader m_clsJSON = new JSONReader();
...
//                GMESSendProc();
        ...
        public int RcvProcEIDS_R()
        {
            ...
            if (m_ProductDataJigA.SetID.Count() > 0 || m_ProductDataOldJigA.SetID.Count() > 0)
            {
                if (m_EIDS_Ack.SETID == m_ProductDataJigA.SetID || m_EIDS_Ack.SETID == m_ProductDataOldJigA.SetID)
                {
            ...
            else if (m_ProductDataJigB.SetID.Count() > 0 || m_ProductDataOldJigB.SetID.Count() > 0)
            {
                if (m_EIDS_Ack.SETID == m_ProductDataJigB.SetID || m_EIDS_Ack.SETID == m_ProductDataOldJigB.SetID)
                {
        ...
        public int RcvProcEPDS()
        {
            ...
            // sjyi 2024.01.15 변수 초기화(이전장비 검사 결과 확인을 위해 추가)
            m_ProductData.strStatorType = "";
            m_ProductData.strWeldEquipID = "";
            m_ProductData.LsSubItem.Clear();        // sjyi 2024.01.15 서브리스트 초기화 필요(이전 Version에도 추가 필요)
            m_ProductData.LsSubWedingInfo.Clear();

            // ProductData 저장
            if (m_clsJSON.GetJSONDatas().Count > 0)
            {
                int nSubItemIndex = 0;
                // sjyi 2024.01.15 변수 추가 및 초기화(전검사 결과 확인을 위해 추가)
                int nWeldItemIndex = 0;
                bool bFindModelInfo = false;
                bool bFindWeldEquipInfo = false;
                bool bSlotLayer = false;
                bool bFindResult = false;

                string strWeldInfoID = "";
                    ...
                    else if (Tmp.ParaentNode == "SUBITEM_LIST")
                    {
                        if (Tmp.NodeName == "NAME")
                        {
                            ...
                            switch (Tmp.Value)
                            {
                                case "MODEL_INFO":
                                    ...
                                    break;
                                case "WELD_EQPID":
                                    bFindWeldEquipInfo = true;
                                    break;
                                case "SPEC_ITEM_CATEGORY_ID":
                                    break;
                                case "PRODCODE":
                                    break;
                                case "PALLETQTY":
                                    break;
                                case "GMES_CODETYPE_IN":
                                    break;
                                case "KALS_PALLET_SN_TYPE":
                                    break;
                                case "KALS_BOX_SN_TYPE":
                                    break;
                                case "GMES_ORIGIN_INSP_JUDGE_CODE":
                                    break;
                                case "MODEL_DESC":
                                    break;
                                case "KALS_LS_LABEL_TYPE":
                                    break;
                                case "PROGRAM_ID":
                                    break;
                                case "BOXQTY":
                                    break;
                                case "KALS_LS_SN_TYPE":
                                    break;
                                case "RESULT":
                                    bFindResult = true;
                                    break;
                                default:
                                    {
                                        if(Tmp.Value.Length == 6 && Tmp.Value.IndexOf('-') >= 0)
                                        {
                                            bSlotLayer = true;
                                        }
                                        else
                            ...
                            // sjyi 2024.01.15 추가(용접 장비 ID 확인)
                            if (bSlotLayer == true)
                            {
                                strWeldInfoID = Tmp.Value;
                            }
                        }
                        else if (Tmp.NodeName == "VALUE")
                        {
                            ...
                            if (bFindModelInfo)
                            {
                                ...
                                bFindModelInfo = false;
                            }

                            // sjyi 2024.01.15 추가(용접 장비 ID가 있으면, 이전장비 용접 결과 저장) -->
                            if (bFindWeldEquipInfo == true && m_ProductData.strWeldEquipID.Length == 0) // sjyi 2024.01.15 추가(용접 장비 ID 확인)
                            {
                                m_ProductData.strWeldEquipID = Tmp.Value;
                                bFindWeldEquipInfo = false;
                            }

                            // sjyi 2024.05.09, 이전장비 검사 결과 추가
                            if(bFindResult == true)
                            {
                                m_ProductData.strInspResult = Tmp.Value;
                                bFindResult = false;
                            }
                            ...
                            if (bSlotLayer)
                            {
                                if(strWeldInfoID.Length > 0) // 전검사 결과 저장
                                {
                                    ProductWeldingInfo ItemResult = new ProductWeldingInfo();

                                    ItemResult.strWeldInfoName = strWeldInfoID; //SlotNo, Layer No
                                    ItemResult.strWeldInfoResult = Tmp.Value; // 검사 결과

                                    m_ProductData.LsSubWedingInfo.Add(ItemResult);
        ...
        public void SetProductData(int nJigNo, EPDS stProductData)
        {
            if (nJigNo == 0) // Jig A
            {
                ...
                m_ProductDataJigA.strWeldEquipID = stProductData.strWeldEquipID; // sjyi 2024.01.15, 용접정보 추가
                m_ProductDataJigA.LsSubWedingInfo = stProductData.LsSubWedingInfo; // sjyi 2024.01.15, 용접정보 추가
            }
            else if (nJigNo == 1) // Jig B
            {
                ...
                m_ProductDataJigB.strWeldEquipID = stProductData.strWeldEquipID; // sjyi 2024.01.15, 용접정보 추가
                m_ProductDataJigB.LsSubWedingInfo = stProductData.LsSubWedingInfo; // sjyi 2024.01.15, 용접정보 추가, 재용접기는 Jig B는 없으나, 만약을 위해 추가
            }
        }
...
//                m_ProductDataJigA.LsSubItem     = m_ProductData.LsSubItem;
        ...
        public void SetOldProductData(int nJigNo, EPDS stProductData)
        {
            if (nJigNo == 0) // Jig A
            {
                ...
                m_ProductDataOldJigA.strWeldEquipID = stProductData.strWeldEquipID; // sjyi 2024.01.15, 용접정보 추가
            }
            else if (nJigNo == 1) // Jig B
            {
                ...
                m_ProductDataOldJigB.strWeldEquipID = stProductData.strWeldEquipID; // sjyi 2024.01.15, 용접정보 추가
            }
        }
        ...
        public void SendEIDS(List<InspDataGMES> InsptData, string strModelNo, int nReworkCnt, string strBarCode, string strJigType, EPDS producData, EAYT_R EQPData, string strFinalJudgement, int nJigNo)
        {
            // Make Json File
            string strPath = Application.StartupPath + "\\EIDS.json";
            if(nJigNo == 0)
            {
                m_clsJSON.MakeEIDS(strPath, InsptData, strModelNo, nReworkCnt, strBarCode, strJigType, m_ProductDataJigA, EQPData, strFinalJudgement);
            }
            else
            {
                m_clsJSON.MakeEIDS(strPath, InsptData, strModelNo, nReworkCnt, strBarCode, strJigType, m_ProductDataJigB, EQPData, strFinalJudgement);
            }
