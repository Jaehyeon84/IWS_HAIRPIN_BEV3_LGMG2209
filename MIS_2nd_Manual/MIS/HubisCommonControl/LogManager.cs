...
namespace HubisCommonControl
{
    ...
    public class LogManager
	{
        ...
        private const string ProductLogFloder = "ProductLog";
        ...
        private static string m_strProductLogFolder;
        #endregion
        ...
        private static string ProductBaseFolder(string strProductPath)
        {
            if (m_strProductLogFolder == null)
            {
                m_strProductLogFolder += strProductPath;
                //사용자가 IWS 셋팅 시 마지막에 폴더 구분(\\) 안했을 경우 가정해서 검사..
                string tmp = strProductPath.Substring(strProductPath.Length - 1, 1);
                if (tmp != "\\")
                {
                    
                    m_strProductLogFolder += "\\";
                }

                m_strProductLogFolder += ProductLogFloder;

                if (!Directory.Exists(m_strProductLogFolder))
                    Directory.CreateDirectory(m_strProductLogFolder);
            }

            return m_strProductLogFolder;
        }
        ...
        /// 생산 로그 기록을 위한 로그 파일
        /// </summary>
        public static string ProductLogFilePath(string strProductPath)
        {           
                return string.Format("{0}\\{1:yyyy-MM-dd}.log", ProductBaseFolder(strProductPath), DateTime.Now);
    }
    ...
    public static void WriteLog (LogType logType, string logContents)
        ...
        public static void ProductWriteLog(LogType logType, string logContents, string ProductPath)
        {
            if (false == string.IsNullOrEmpty(logContents))
            {
                string strType = "Info";
                ProductWriteLogFile(string.Format("[{0:yyyy-MM-dd HH:mm:ss.fff}]\t[{1}]\t{2}", DateTime.Now, strType, logContents), ProductPath);
            }
        }
        ...
        /// 생산 로그 기록을 위한 함수
        /// </summary>
        /// <param name="strLogContents"></param>
        private static void ProductWriteLogFile(string strLogContents, string strProductPath)
        {
            StreamWriter writer = null;
            try
            {
                writer = new StreamWriter(ProductLogFilePath(strProductPath), true, Encoding.Default);
                writer.Write(strLogContents);
                writer.WriteLine();
            }
            catch
