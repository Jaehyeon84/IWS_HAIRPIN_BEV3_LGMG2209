...
void CDlgHairpinPattern::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_USE_SLOTMODE, m_bUseSlotMode);
	DDX_Control(pDX, IDC_EDIT_SLOT_NO_1, m_editSlotNo1st);
	DDX_Control(pDX, IDC_EDIT_SLOT_NO_2, m_editSlotNo2nd);
}
...
BEGIN_MESSAGE_MAP(CDlgHairpinPattern, CDialogEx)
	...
	ON_BN_CLICKED(IDC_CHECK_USE_SLOTMODE, &CDlgHairpinPattern::OnBnClickedCheckUseSlot)
END_MESSAGE_MAP()
...
// 	m_pChartProfile1st.ShowWindow(SW_SHOW);
	...
	m_editSlotNo1st.SetValue(0);
	m_editSlotNo2nd.SetValue(0);
...
void CDlgHairpinPattern::OnBnClickedOk()
{
	...
	if (m_bUseSlotMode)
	{
		m_n1STSlot = m_editSlotNo1st.GetValue();
		m_n2NDSlot = m_editSlotNo2nd.GetValue();
	}
...
void CDlgHairpinPattern::OnBnClickedCheckUseSlot()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	UpdateData();
	UpdateUI_SlotNode();	
...
void CDlgHairpinPattern::UpdateUI_SlotNode()
{
	if (m_bUseSlotMode) // Slot Mode
	{
		int nUID1 = IDC_RADIO_1ST_1;
		int nUID2 = IDC_RADIO_2ND_1;
		for (int i = 0; i < 7; i++)
		{
			GetDlgItem(nUID1 + i)->EnableWindow(FALSE);
			GetDlgItem(nUID2 + i)->EnableWindow(FALSE);
		}
		GetDlgItem(IDC_EDIT_SLOT_NO_1)->EnableWindow(TRUE);
		GetDlgItem(IDC_EDIT_SLOT_NO_2)->EnableWindow(TRUE);
	}
	else // Layer Mode
	{
		int nUID1 = IDC_RADIO_1ST_1;
		...
		for (int i = 0; i < 7; i++)
		{
			GetDlgItem(nUID1 + i)->EnableWindow(TRUE);
			GetDlgItem(nUID2 + i)->EnableWindow(TRUE);
		}
		GetDlgItem(IDC_EDIT_SLOT_NO_1)->EnableWindow(FALSE);
		GetDlgItem(IDC_EDIT_SLOT_NO_2)->EnableWindow(FALSE);
	}
}
