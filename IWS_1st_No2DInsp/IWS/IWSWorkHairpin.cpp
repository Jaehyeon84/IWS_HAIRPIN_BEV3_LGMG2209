...
//#include "../Hairpin2DInsp/Hairpin2D.h" // ehjang 2D Insp. 추가 //gseom_240704_2DInsp 주석 추가
...
//			if (!((idx + 1) % MAX_NUM_GRP)) 
			...
			if (nMaxNumGrp == 3) // sjyi 2nd 패턴이 zigzag2가 아님 기존과 동일
			{
				...
				{ 
					...
					{
						case LC_ENT_ELLIPSE: 
							AlignEntityEllipse1st(Ent, &((CJobWeld*)p)->m_iPenID); 
							((CJobWeld*)p)->m_nShape1st = IWS_1ST_ELLIPSE;
							RotateHairpin(Ent, ((CJobWeld*)p)->GetRotation());
							break;
						case LC_ENT_LINE: 
							AlignEntityLine1st(Ent, &((CJobWeld*)p)->m_iPenID); 
						...
						case LC_ENT_SPIRAL: // sjyi 2024.02.01 Spiral 추가
							AlignEntitySpiral1st(Ent, &((CJobWeld*)p)->m_iPenID);
...
void CIWSWorkHairpin::AlignEntitySpiral1st(CEntity *pEnt, int *penID)
{
	CEntitySpiral *e = (CEntitySpiral*)pEnt;
	double hp_cx, hp_cy;
	GetHairpinCenter(hp_cx, hp_cy);
