...
CDlgHairpinOffset::CDlgHairpinOffset(CWnd* pParent /*=nullptr*/)
	...
	, m_dCenOffX(0)
	, m_dCenOffY(0)
	//, m_b3DUse(FALSE) //gseom_240705_2DInsp 주석 추가
	, m_dEditAllNum(0)
{
}
...
void CDlgHairpinOffset::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_ALL_NUM, m_dEditAllNum);
}


BEGIN_MESSAGE_MAP(CDlgHairpinOffset, CDialogEx)
	ON_BN_CLICKED(IDOK, &CDlgHairpinOffset::OnBnClickedOk)
	//ON_BN_CLICKED(IDC_RADIO_3D, &CDlgHairpinOffset::OnClickedRadioRcp3d)//gseom_240705_2DInsp 주석 추가
	//ON_BN_CLICKED(IDC_RADIO_2D, &CDlgHairpinOffset::OnClickedRadioRcp2d)//gseom_240705_2DInsp 주석 추가
	ON_BN_CLICKED(IDC_BUTTON_ALL_SET, &CDlgHairpinOffset::OnBnClickedButtonAllSet)
	//ON_BN_CLICKED(IDC_BUTTON_RCP_ENABLE, &CDlgHairpinOffset::OnBnClickedButtonRcpEnable)//gseom_240705_2DInsp 주석 추가
END_MESSAGE_MAP()
...
BOOL CDlgHairpinOffset::OnInitDialog()
{
	...
	GetDlgItem(IDC_RADIO_3D)->EnableWindow(FALSE);
	GetDlgItem(IDC_RADIO_2D)->EnableWindow(FALSE);*/
...
void CDlgHairpinOffset::InitGrid()
{
	...
	/*if (m_b3DUse == TRUE)
	{
		((CButton *)GetDlgItem(IDC_RADIO_3D))->SetCheck(TRUE);
		((CButton *)GetDlgItem(IDC_RADIO_2D))->SetCheck(FALSE);
		//gseom_240705_2DInsp 주석 추가<--
	}*/
	for (int i = 0; i < m_nSlot * m_nLayer; i++)
	{
	...
	/*else
	{
		...
		for (int i = 0; i < m_nSlot * m_nLayer; i++)
		{
			m_grid.SetDouble(0, (long)i, m_hpOffset2D[i].ax);
			m_grid.SetDouble(1, (long)i, m_hpOffset2D[i].ay);
			m_grid.SetDouble(2, (long)i, m_hpOffset2D[i].bx);
			m_grid.SetDouble(3, (long)i, m_hpOffset2D[i].by);
		}
	}*/
	//gseom_240705_2DInsp 주석 추가<--
	m_grid.RedrawAll();
...
//gseom_240705_2DInsp 주석 추가-->
/*void CDlgHairpinOffset::OnClickedRadioRcp3d()
{
	m_b3DUse = TRUE;
	InitGrid();
	UpdateData(FALSE);
...
void CDlgHairpinOffset::OnClickedRadioRcp2d()
{
	m_b3DUse = FALSE;
	InitGrid();
	UpdateData(FALSE);
...
void CDlgHairpinOffset::OnBnClickedButtonAllSet()
{
	UpdateData(TRUE);
	//gseom_240705_2DInsp 주석 추가
	//if (m_b3DUse)
	...
	for (int i = 0; i < m_nSlot * m_nLayer; i++)
	{
		m_hpOffset[i].ax = m_dEditAllNum;
		m_hpOffset[i].ay = m_dEditAllNum;
		m_hpOffset[i].bx = m_dEditAllNum;
		m_hpOffset[i].by = m_dEditAllNum;
	}
	//}
	//gseom_240705_2DInsp 주석 추가-->
	/*else
	{
		for (int i = 0; i < m_nSlot * m_nLayer; i++)
		{
			m_hpOffset2D[i].ax = m_dEditAllNum;
			m_hpOffset2D[i].ay = m_dEditAllNum;
			m_hpOffset2D[i].bx = m_dEditAllNum;
			m_hpOffset2D[i].by = m_dEditAllNum;
		}
	}*/
	//gseom_240705_2DInsp 주석 추가<--
	InitGrid();
...
//gseom_240705_2DInsp 주석 추가-->
/*void CDlgHairpinOffset::OnBnClickedButtonRcpEnable()
{
	CString strEnable = _T("Enable");
	if (m_bEnable == FALSE)
	{
		...
		GetDlgItem(IDC_RADIO_3D)->EnableWindow(TRUE);
		GetDlgItem(IDC_RADIO_2D)->EnableWindow(TRUE);
	}
	else
	{
		m_bEnable = FALSE;
