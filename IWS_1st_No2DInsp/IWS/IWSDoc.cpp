...
CIWSDoc::CIWSDoc() noexcept
{
	...
	//m_pHairpin2D = new CHairpin2D; // ehjang 2D Insp. 추가 //gseom_240704_2DInsp 주석 추가
...
#define RESPONSE_TIMEOUT 3000
	...
	while (m_bRunInterfaceThread)
	{
		...
		case WAIT_TIMEOUT:
			...
			//gseom_240704_2DInsp 주석 추가<--
			//else if (bUseAllwaysScan && g_pGrabImgDlg->m_bDoScan) gseom_240704_2DInsp 주석 추가로 코드수정
			if (bUseAllwaysScan && g_pGrabImgDlg->m_bDoScan)
			{
				dwTimeLink = dwCurr;
				m_status.bOk[IWS_PLC] = TRUE;
			}
			...
			if (theApp.m_bWorkOn)
			{
				...
				//else if (bUseAllwaysScan == FALSE && g_pGrabImgDlg->m_bDoScan == FALSE) //gseom_240704_2DInsp 주석추가로 코드 수정
				if (bUseAllwaysScan == FALSE && g_pGrabImgDlg->m_bDoScan == FALSE)
				{
					m_plc.SetWeldReady(FALSE);
				}
...
void CIWSDoc::OnFileHpConfig()
{
	...
	//dlg.m_b3DUse = TRUE; //gseom_240705_2DInsp 주석 추가
...
void CIWSDoc::SaveInspectData(TCHAR* szFilePath)
{
	...
	_tracer(TRACE_NORMAL, 1, _T("File Save Fail Inspect Data. %s"), szFilePath);
