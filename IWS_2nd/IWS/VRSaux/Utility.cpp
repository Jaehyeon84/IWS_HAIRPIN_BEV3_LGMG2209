...
void CUtility::DrawCrossOverlay(int width, int height, int crossSize, int windownumber, RGBValue color)
{
	Point horiPt0[2];
...
void CUtility::DrawCrossLines(Image* pImg, int width, int height, int crossSize, RGBValue color)
{
	Point horiPt0[2];
	horiPt0[0].x = (width - crossSize) / 2;
	horiPt0[1].x = (width + crossSize) / 2;
	...
	imaqOverlayLine(pImg, horiPt0[0], horiPt0[1], &color, NULL);
	imaqOverlayLine(pImg, vertPt0[0], vertPt0[1], &color, NULL);
	...
	rtPt[2].y = rtPt[3].y = height - 1;
	for (int j = 0; j < 4; j++)
		imaqOverlayLine(pImg, rtPt[j], rtPt[(j + 1) % 4], &color, NULL);
}
...
void CUtility::DrawTextOverlay(Image* pImg, Point pos, double dispRate, double acqRate, RGBValue color)
{
	...
	strAvgFrmRate.Format(L" FR:%2d", int(acqRate));
	...
	options.fontName = (CStringA)(L"Aerial");
	options.fontSize = 10;

	imaqOverlayText(pImg, pos, (CStringA)strAvgFrmRate, &rgb, &options, NULL);
}

void CUtility::DrawTextOverlay2(Image* pImg, Point pos, double dispRate, double acqRate, RGBValue color)
{
	CString strAvgFrmRate;
	//strAvgFrmRate.Format(L" FR:%2d", int(acqRate));
	strAvgFrmRate.Format(L"");
