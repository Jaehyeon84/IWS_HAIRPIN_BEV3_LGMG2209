...
HANDLE		g_hEventPushQueueDone2nd;
...
CGrabImgDlg::CGrabImgDlg(CIWSDoc *pDoc, CWnd* pParent /*=NULL*/)
	: CDialogEx(IDD_GRAB_DLG, pParent)
{
	...
	m_hEventInitDone = ::CreateEvent(NULL, FALSE, FALSE, NULL);
	...
	/* 2nd CCD */
	m_nCamBuf2nd = NUM_INTERNAL_CAMBUF;
	// for grab thread
	g_ImgType2nd = IMAQ_IMAGE_U8;
	g_bGrabThreadAlive2nd = FALSE;
	g_hEventPushQueueDone2nd = ::CreateEvent(NULL, FALSE, FALSE, NULL);
	...
	if (Is1stGrabThreadInitDone())
	{
		g_bCameraStateOK2nd = FALSE;
		m_pGrapThread2nd = AfxBeginThread(GrabImgThread2nd, (LPVOID)this, THREAD_PRIORITY_BELOW_NORMAL);
	}
...
CGrabImgDlg::~CGrabImgDlg()
{
	...
	//if (!g_bCameraStateOK2nd) // 카메라 강제 종료
	//	Pylon::PylonTerminate();

	dwRes = WaitForSingleObject(m_hEventGrabThreadEnd2nd, 3000);		// wait 3 second
	if (dwRes == WAIT_TIMEOUT)	// Grab Thread 강제 종료
	{
	...
	::CloseHandle(g_hEventPushQueueDone2nd);
...
BOOL CGrabImgDlg::Is1stGrabThreadInitDone()
{
	DWORD ret = ::WaitForSingleObject(m_hEventInitDone, 10000);
...
void CGrabImgDlg::AcqProcessStart()
{

}

void CGrabImgDlg::GrabimgThreadBody()
{
	Sleep(200);
	g_Util.Logging(L"-- 1st Grab Thread Started ! -- \n");
	...
	try
	{
		...
		if (!pTl)
		{
			...
			g_bCameraStateOK = FALSE;
			//SetEvent(m_hEventInitDone);
			return;
		}

		// Get all attached cameras and exit application if no camera is found
		//DeviceInfoList_t devices;
		int nDevices = TlFactory->EnumerateDevices(devices);
		TRACE("\t\t Number of camera: %d\n", nDevices);
		if (0 == nDevices)
		{
		...
		m_Camera[0] = new Camera_t(pTl->CreateDevice(devices[0]));
		Camera_t* Camera = m_Camera[0];

		std::string modelname, strIP, gw;
		CDeviceInfo di = Camera->GetDeviceInfo();
		modelname = di.GetModelName();
		strIP = di.GetIpAddress();
		TRACE("\t\t 1st Grab Thread, 1st camera ip: %s\n", strIP.c_str());
		...
		for (std::vector<CGrab2DBuff*>::const_iterator x = BufferList.begin(); x != BufferList.end(); ++x)
		{
			// Put buffer into the grab queue for grabbing
			StreamGrabber.QueueBuffer((*x)->GetBuffHandle(), NULL);
		}
		...
		while (g_bGrabThreadAlive)
		{
			...
			if (g_bLiveDisplay)
			{
				...
				ret = ::WaitForSingleObject(g_hEventSrtCopyDispData, 30);
				if (!m_bDoScan)
				{
					grab_status = Grab(&StreamGrabber, &grabResult);
					if(grab_status == Failed || grab_status == Queued || grab_status == Canceled)
						TRACE(L"\t\t-------- CAMERA 0: grab failed !\n");
					bGrabTried = TRUE;
				}
			...
			if (bGrabTried && grab_status == Grabbed)
			{
				StreamGrabber.QueueBuffer(grabResult.Handle(), NULL);
				bGrabTried = FALSE;
			}
		...
		for (std::vector<CGrab2DBuff*>::iterator it = BufferList.begin(); it != BufferList.end(); it++)
		{
			StreamGrabber.DeregisterBuffer((*it)->GetBuffHandle());
			delete *it;
			*it = NULL;
		}
	...
	g_Util.Logging(L"-- 1st Grab Thread exited ! -- \n");
}
...
/* 2nd CCD */
UINT CGrabImgDlg::GrabImgThread2nd(LPVOID pArgs)
{
	CGrabImgDlg* me = (CGrabImgDlg*)pArgs;
...
void CGrabImgDlg::GrabimgThreadBody2nd()
{
	//Sleep(200);
	g_Util.Logging(L"-- 2nd Grab Thread Started ! -- \n");

	try
	{
		m_Camera[1] = new Camera_t(pTl->CreateDevice(devices[1]));
		Camera_t* Camera = m_Camera[1];

		std::string modelname, strIP, gw;
		CDeviceInfo di = Camera->GetDeviceInfo();
		modelname = di.GetModelName();
		strIP = di.GetIpAddress();
		TRACE("\t\t 2nd Grab Thread: 2nd camera ip: %s\n", strIP.c_str());
		INodeMap* pTlNodeMap = Camera->GetTLNodeMap();
		if (NULL == pTlNodeMap)
		{
			g_Util.Logging(L"This camera class doesn’t provide a transport layer nodemap");
		...
		// We won't queue more than c_nBuffers image buffers at a time
		StreamGrabber.MaxNumBuffer.SetValue((uint32_t)m_nCamBuf2nd);
		...
		while (g_bGrabThreadAlive2nd)
		{
			if (g_bLiveDisplay2nd)
			{
				...
				ret = ::WaitForSingleObject(g_hEventSrtCopyDispData2nd, 5);
				grab_status = Grab2nd(&StreamGrabber, &grabResult);
				if (grab_status == Failed || grab_status == Queued || grab_status == Canceled)
					TRACE(L"\t\t-------- CAMERA 1: grab failed !\n");
				...
				case WAIT_OBJECT_0:
					if (grab_status == Grabbed)
					{
						...
						if (g_ImgType2nd == IMAQ_IMAGE_U8)
						{
							for (UINT j = 0; j < m_nGrabHt2nd; j += 2)
							{
								for (UINT i = 0; i < m_nGrabWd2nd; i += 2)
								{
									*d = *s;
									d++;
									s += 2;
					...
					// display on Ni window
					//::PostMessage(m_pLiveCCDDlg->m_hWnd, UWM_DISPIMG, 0, 0);
			...
			if (m_bSnapImg2nd)
			{
				if (!bGrabTried)
				{
					grab_status = Grab2nd(&StreamGrabber, &grabResult); //DHChoi 20200630
					bGrabTried = TRUE;
				}
				...
				if (grab_status == Grabbed)
				{
					g_bCameraStateOK2nd = TRUE;
					pImageBuffer = (uint8_t *)grabResult.Buffer();
					memcpy(m_pSnapData2nd, pImageBuffer, sizeof(unsigned char) * ImageSize);
					m_bSnapImg2nd = FALSE;
				}
				else
				{
					g_bCameraStateOK2nd = FALSE;
					memcpy(m_pSnapData2nd, 0, sizeof(unsigned char) * ImageSize);
					g_Util.Logging(L"Grab in Snap Process Failed !\n");
				}

				::SetEvent(m_hEventSnapimg2nd);
			}
	...
	g_Util.Logging(L"-- 2nd Grab Thread exited ! -- \n");
}


EGrabStatus CGrabImgDlg::Grab2nd(Camera_t::StreamGrabber_t *pStreamGrabber, GrabResult* pGrabResult)
{
	// Wait for the grabbed image with timeout of 300 seconds
	pStreamGrabber->GetWaitObject().Wait(30000);
...
BOOL CGrabImgDlg::ReadImageOnDisk2nd(Image* pImg)
{
...
void CGrabImgDlg::SnapAndSaveImg2nd(CString strPath, CString strImgName)
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	m_bSnapImg2nd = TRUE;
...
unsigned char* CGrabImgDlg::SnapImg2nd()
{
	m_bSnapImg2nd = TRUE;
