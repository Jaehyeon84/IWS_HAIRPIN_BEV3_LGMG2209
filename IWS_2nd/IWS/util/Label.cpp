...
// 1.1
	...
	m_bInvalidate = TRUE;	// blue 2009.09.14 
}
...
void CLabel::UpdateSurface()
{
	...
	GetParent()->InvalidateRect(rc, m_bInvalidate);	// blue 2009.09.14 org : TRUE
...
//	GetWindowText(strText);
	...
	CString strText;
	if(m_bInvalidate)	GetWindowText(strText);
	else				strText = m_strText;	// blue 2009.09.14 
	...
	if (!m_bTransparent)
	{
		...
		pDCMem->DeleteDC();// cdh 2008.08.20 memory leck
		delete pDCMem;
	}
}
...
CLabel& CLabel::SetText(const CString& strText)
{
	...
	{
		if(m_bInvalidate)	SetWindowText(strText);
		else				m_strText = strText;	// blue 2009.09.14 
		UpdateSurface();
	}
