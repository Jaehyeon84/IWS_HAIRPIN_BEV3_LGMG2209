...
int CJobWeld::DoJob(CIWSDoc *pDoc, BOOL bSimulation, int nRepeat, double *pRptProfile)
{
	...
	if (bSimulation)
	{
		pPen = &(*m_pPens)[14]; //int isimPen = (int)m_pPens->size() - 2;
	}
	else
	{
		pPen = &(*m_pPens)[m_iPenID];
...
//			if (pRptProfile)
	...
	m_pScan->Execute();
	m_pScan->WaitMoving();*/
	//m_pScan->INT_AO_SetPower(0);
	//_tracer(TRACE_NORMAL, 1, L"Pen[%d], DoJobPower = %f, N-path = %d", m_iPenID, bSimulation ? 0 : pPen->m_dblPower, nPath);
	return 0;
}
...
BOOL CJobWeld::LoadIndividualPower(CFile *pf)
{
	...
	{
		nRtn = pf->Read(&stRepeatInfo.dPower, sizeof(double));
