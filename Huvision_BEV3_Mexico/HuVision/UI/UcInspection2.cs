...
{
    public partial class UcInspection2 : UserControl
    {
        ...
        private short[] inspectionResultArr;    // 용접 후 검사 결과
        ...
        public UcInspection2()
        {
            InitializeComponent();

            iniManagement = IniManagement.Instance();

            inspectionResultArr = new short[iniManagement.inspectionModel.WeldAfterInspect_NumberofSlots * (iniManagement.inspectionModel.WeldAfterInspect_NumberofLayers / 2)];
        ...
        private void InitGridControl()
        {
            ...
            CreateColumn("Result", "Result", 55, dataTable, gridView, typeof(int));                    // Result

            SetColumn(iniManagement.iniModel.Coulmn_Col1);
            SetColumn(iniManagement.iniModel.Coulmn_Col2);
            SetColumn(iniManagement.iniModel.Coulmn_Col3);
            SetColumn(iniManagement.iniModel.Coulmn_Col4);
            SetColumn(iniManagement.iniModel.Coulmn_Col5);
            SetColumn(iniManagement.iniModel.Coulmn_Col6);
            SetColumn(iniManagement.iniModel.Coulmn_Col7);
            SetColumn(iniManagement.iniModel.Coulmn_Col8);
            SetColumn(iniManagement.iniModel.Coulmn_Col9);
            SetColumn(iniManagement.iniModel.Coulmn_Col10);
        ...
        private void SetColumn(string value)
        {
            ...
            CreateColumn(tokken[0], tokken[0], int.Parse(tokken[1]), dataTable, gridView, typeof(string));
        }
        ...
        {
            pnl_Image.BackgroundImage = Properties.Resources.noImage;
            pnl_Image.Refresh();
        ...
        public bool Processing(string imgPath, string dataPath, string lotID,int modelNo)
        {

            if (!FileSearch(imgPath, dataPath, 2000))
                return false;
            ...
            try
            {
                ...
                string args = string.Format("{0} {1}", dataPath, modelNo);
                AppendLog("Args: " + args);
            ...
            if (!FileSearch(imgPath, dataFile, iniManagement.iniModel.Process_AfterTimeOut))
            {
            ...
            try
            {
                ...
                string saveCsvPath = string.Format("{0}_{1}.csv", imgPath.Remove(imgPath.Length - 4), result);
        ...
        private void CsvParsing(string csvFile)
        {
            ...
            while (!sr.EndOfStream)
            {
                ...
                for (int i = (int)COL.Result; i < (int)COL.Radial; i++)
                    dataTable.Rows[rowIndex][i] = tokken[i];
        ...
        private void FileDelte(string dataFile)
        {
            int count = 0;
            while (true)
            {
        ...
        public byte[] GetWriteResultData(List<BefInspModel> befInspModelList)
        {
            try
            {
        ...
        private void Draw(Graphics g = null)
        {
            ...
            {
                if (g == null)
                    g = pnl_Image.CreateGraphics();

                double angle = 360.0 / iniManagement.inspectionModel.WeldAfterInspect_NumberofSlots;
                double x, y;

                float centerX = pnl_Image.Width / 2;
                float centerY = pnl_Image.Height / 2;

                for (int i = 0; i < iniManagement.inspectionModel.WeldAfterInspect_NumberofSlots; i++)
                {
                    double radius = iniManagement.iniModel.Draw_Range / 2.0;
                    double dA = 90 + i * angle + iniManagement.iniModel.Draw_Angle;
                    ...
                    g.DrawString((i + 1).ToString(), drawFont, drawResultArr[i] == (int)RESPONSE.OK ? okBrush : ngBrush,
                        (float)x + centerX + iniManagement.iniModel.Draw_XOffSet, -(float)y + centerY + iniManagement.iniModel.Draw_YOffSet);
                }
