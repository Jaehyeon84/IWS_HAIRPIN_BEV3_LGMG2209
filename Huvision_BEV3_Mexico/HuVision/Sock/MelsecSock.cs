...
namespace HuVision.Sock
{
    class MelsecSock : CommonSock
    {
        ...
        public override byte[] Read(string area, int startAddr, int count)
        {
            ...
            {
                LogManager.WriteLog(LogType.Error, "MelsecSock_Read: " + e);
                Close();
                return null;
            }
        }
        ...
        public override bool Write(string area, int startAddr, int count, byte[] data)
        {
            try
            {
                ...
                socket.Receive(new byte[100]);

                int result = socket.Receive(new byte[100]);
                if (result == -1)
                {
                    LogManager.WriteLog(LogType.Error, "MelsecSock Write -1: " + result);
                    return false;
                }
            ...
            {
                LogManager.WriteLog(LogType.Error, "MelsecSock_Write: " + e);
