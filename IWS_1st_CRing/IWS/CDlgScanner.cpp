...
void CDlgScanner::OnBnClickedMfcbuttonScannerApply()
{
	...
	if ((m_pCorrParam->dblOffsetX / dScaleFactor) != dOffsetX) {
		_tracer(TRACE_NORMAL, 2000, L"Scale Offset (%d) : %.3f -> %.3f", m_iCurrentLens, (m_pCorrParam->dblOffsetX / dScaleFactor), dOffsetX);
		m_pCorrParam->dblOffsetX = dOffsetX * dScaleFactor;
	}
	if ((m_pCorrParam->dblOffsetY / dScaleFactor) != dOffsetY) {
		_tracer(TRACE_NORMAL, 2000, L"Scale Offset (%d) : %.3f -> %.3f", m_iCurrentLens, (m_pCorrParam->dblOffsetY / dScaleFactor), dOffsetY);
