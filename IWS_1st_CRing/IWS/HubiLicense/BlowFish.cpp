...
#pragma warning(disable: 4097)		// YR
...
const SBlock CBlowFish::sm_chain0 = SBlock(0UL,0UL);
...
void CBlowFish::Initialize(char const* keydata, int keydatalength, SBlock const& roChain,
	int iMode, int iPadding)
{
	...
	if (true == m_bInit)
	{
		...
		if (0 == memcmp(m_pKey, key, m_keylength))
			bSameKey = true;
	}
	if (true == bSameKey)
		//Fast Initialization
		return;
	if (NULL == m_pKey)
		m_pKey = (char*) (new char[m_keylength]);
	memcpy(m_pKey, key, m_keylength);
...
void CBlowFish::Signature(char* pcSig)
{
	//8+56+1+1+1
	char acSigData[68] = {0};
	strcat_s(acSigData, "BLOWFISH");
	int iLen = strlen(acSigData);
	memcpy(acSigData+iLen, m_pKey, m_keylength);
	sprintf_s(acSigData+iLen+m_keylength, 200,"%d%d", m_iMode, m_iPadding);
	CSHA oSHA;
