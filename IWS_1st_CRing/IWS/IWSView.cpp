...
void CALLBACK eventEntPropChanged(HANDLE hEvent) {
	...
	int nEntType = lcPropGetInt(hEntity, LC_PROP_ENT_TYPE);
	if (nEntType == LC_ENT_POLYLINE || nEntType == LC_ENT_LINE || nEntType == LC_ENT_SPIRAL) {
...
void CIWSView::OnButtonHairpin()
{
	...
	pDoc->m_work.m_hp.dPtnRotationAngle, dlg.m_d2ndWeldSizeW2);
