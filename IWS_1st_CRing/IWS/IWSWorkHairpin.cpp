...
void CIWSWorkHairpin::ReadHairpinInfoV103(CFile * pf)
{
	...
	// sjyi 2024.09.29 추가 소티 반복
	m_hp.bEnableSortiRpt = FALSE;
	m_hp.nSortieCnt = 0;
	m_hp.nSortieDir = SORTIE_ONEWAY;

	m_hp.HpSize2nd2 = 0.0f;
	m_hp.bUseOutToInP1 = TRUE;
	m_hp.bUseOutToInP2 = TRUE;
...
void CIWSWorkHairpin::ReadHairpinInfoV107(CFile * pf)
{
	IWS_HAIRPIN_INFO_V107   hp;
	pf->Read(&hp, sizeof(IWS_HAIRPIN_INFO_V107));
...
void CIWSWorkHairpin::ModifyHairpin(int n1st, int n2nd, BOOL bUpdate, double dRotateAngle, double dRotateAngle2, BOOL bKeepRotation)
{
	// 0 = All, 1~4 = Layer
...
void CIWSWorkHairpin::GenHairPin(int nSlot, int nLayer, double startAng,
	...
	double dRotateAngle, double HpSize2nd_2)
{
	double cx, cy;
