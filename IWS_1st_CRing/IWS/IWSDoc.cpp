...
BOOL CIWSDoc::Update3DPosition()
{
	...
	for (int i = 0; i < nWordCnt; i += 16)
	{
		...
		if ((m_plcData->m_rDataInsp[i] == 1) || m_plcData->m_rDataInsp[i] == 2)
		{
			...
			//RCP 1 - 일괄 용접				//RCP  7 - 일괄 용접
			//RCP 2 - 1-2 Layer 분할 용접		//RCP  8 - 1-2 Layer 분할 용접
			//RCP 3 - 3-4 Layer 분할 용접		//RCP  9 - 3-4 Layer 분할 용접
			//RCP 4 - 5-6 Layer 분할 용접		//RCP 10 - 5-6 Layer 분할 용접
			//RCP 5 - 7-8 Layer 분할 용접		//RCP 11 - 7-8 Layer 분할 용접
			//RCP 6 - 9-10 Layer 분할 용접	//RCP 12 - 9-10 Layer 분할 용접
...
#define RESPONSE_TIMEOUT 3000
	...
	while (m_bRunInterfaceThread)
	{
		...
		case WAIT_TIMEOUT:
			...
			if (GetAllwaysScan() && g_pGrabImgDlg->m_bDoScan)
			{
				dwTimeLink = dwCurr;
				m_status.bOk[IWS_PLC] = TRUE;
			}
			else if (GetAllwaysScanAftWelding() && g_pGrabImgDlg->m_bDoScan)
			{
				dwTimeLink = dwCurr;
				m_status.bOk[IWS_PLC] = TRUE;
			}
...
void CIWSDoc::SaveImageAndPos()
{
	...
	// 텍스트 출력
	int textOptions1[5] = { 120, 1, 0, 0, 0 };
	...
	else
	{
		...
		try
		{
			...
			imaqWriteJPEGFile(pImage, CStringA(strFileName), 250, NULL); // sjyi 2024.06.15 파일 포맷 Jpg로 변경
...
BOOL CIWSDoc::GetAllwaysScan()
{
	...
	if (m_plc.GetReWeld() == TRUE) { // Reweld
		if (m_plc.GetWorkJig() == 2) // JIG B
		{
