...
void CPropertiesWnd::ShowJobProperty(IWS_VECTOR_JOB *pJobs)
{
	...
	int nEntityType = LC_ENT_ALL;

	for (auto p : (*pJobs)) {
		m_pJobs.push_back(p);
		bCircle &= (p->GetEntity()->GetEntityType() == LC_ENT_CIRCLE);
		nEntityType &= p->GetEntity()->GetEntityType();
	}
	...
	if (m_pDoc->m_config.m_bUseHairpin)
	{
		pJob = (CJobWeld*)m_pJobs[0];
		str1stLength = CString((_variant_t)pJob->m_d1StLength);
		str1stWidth = CString((_variant_t)pJob->m_d1StWidth);
		str2ndWidth = CString((_variant_t)pJob->m_d2NdWidth);
		str2ndOver = CString((_variant_t)pJob->m_d2NdOver);

		strPower = CString((_variant_t)(*pJob->m_pPens)[pJob->m_iPenID].m_dblPower);
		strJumpSpd = CString((_variant_t)(*pJob->m_pPens)[pJob->m_iPenID].m_dblJumpSpeed);
		strWeldSpd = CString((_variant_t)(*pJob->m_pPens)[pJob->m_iPenID].m_dblMarkSpeed);
	...
	for (auto p : m_pJobs) {
		...
		{
			pJob = (CJobWeld*)p;
			if (str1stLength.GetLength() && str1stLength != CString((_variant_t)pJob->m_d1StLength))  str1stLength = L"";
			if (str1stWidth.GetLength() && str1stWidth != CString((_variant_t)pJob->m_d1StWidth))  str1stWidth = L"";
			if (str2ndWidth.GetLength() && str2ndWidth != CString((_variant_t)pJob->m_d2NdWidth))  str2ndWidth = L"";
			if (str2ndOver.GetLength() && str2ndOver != CString((_variant_t)pJob->m_d2NdOver))  str2ndOver = L"";

			if (strPower.GetLength() && strPower != CString((_variant_t)(*pJob->m_pPens)[pJob->m_iPenID].m_dblPower))  strPower = L"";
			if (strJumpSpd.GetLength() && strJumpSpd != CString((_variant_t)(*pJob->m_pPens)[pJob->m_iPenID].m_dblJumpSpeed))  strJumpSpd = L"";
			if (strWeldSpd.GetLength() && strWeldSpd != CString((_variant_t)(*pJob->m_pPens)[pJob->m_iPenID].m_dblMarkSpeed))  strWeldSpd = L"";
		}
	...
	if (m_pDoc->m_config.m_bUseHairpin)
	{
		...
		m_pItems[PROP_WORK_1ST_LENGTH] = new CMFCPropertyGridProperty(_T("1st Length"), str1stLength, L"1st Length", (DWORD_PTR)PROP_WORK_1ST_LENGTH);
		m_pItems[PROP_WORK_1ST_WIDTH] = new CMFCPropertyGridProperty(_T("1st Width"), str1stWidth, L"1st Width", (DWORD_PTR)PROP_WORK_1ST_WIDTH);
		m_pItems[PROP_WORK_2ND_WIDTH] = new CMFCPropertyGridProperty(_T("2nd Length"), str2ndWidth, L"2nd Length", (DWORD_PTR)PROP_WORK_2ND_WIDTH);
		m_pItems[PROP_WORK_2ND_OVERLEN] = new CMFCPropertyGridProperty(_T("2nd OverLen"), str2ndOver, L"2nd OverLen", (DWORD_PTR)PROP_WORK_2ND_OVERLEN);
		patternSize->AddSubItem(m_pItems[PROP_WORK_1ST_LENGTH]);
		patternSize->AddSubItem(m_pItems[PROP_WORK_1ST_WIDTH]);
		patternSize->AddSubItem(m_pItems[PROP_WORK_2ND_WIDTH]);
		patternSize->AddSubItem(m_pItems[PROP_WORK_2ND_OVERLEN]);

		m_wndPropList.AddProperty(patternSize);

		CMFCPropertyGridProperty* WeldParam = new CMFCPropertyGridProperty(_T("WeldParam"));
		m_pItems[PROP_WORK_PEN_POWER] = new CMFCPropertyGridProperty(_T("Power"), strPower, L"Power", (DWORD_PTR)PROP_WORK_PEN_POWER);
		m_pItems[PROP_WORK_PEN_JUMPSPD] = new CMFCPropertyGridProperty(_T("Jump Speed"), strJumpSpd, L"Jump Speed", (DWORD_PTR)PROP_WORK_PEN_JUMPSPD);
		m_pItems[PROP_WORK_PEN_WELDSPD] = new CMFCPropertyGridProperty(_T("Weld Speed"), strWeldSpd, L"Weld Speed", (DWORD_PTR)PROP_WORK_PEN_WELDSPD);
		WeldParam->AddSubItem(m_pItems[PROP_WORK_PEN_POWER]);
		WeldParam->AddSubItem(m_pItems[PROP_WORK_PEN_JUMPSPD]);
		WeldParam->AddSubItem(m_pItems[PROP_WORK_PEN_WELDSPD]);

		m_wndPropList.AddProperty(WeldParam);
	}
...
#pragma warning(disable:4800)
			...
			if (m_pJobs.size()) {
				...
				for (auto p : m_pJobs) {
					...
					case PROB_WORK_CIRCLE_DIR:
						...
						pJob->GetEntity()->UpdateEntity(dwData, value);
						break;
					case PROP_WORK_1ST_LENGTH:
						pJob->m_d1StLength = _tcstof(CString(value), NULL);
						bUdatePtn = TRUE;
						break;
					case PROP_WORK_1ST_WIDTH:
						pJob->m_d1StWidth = _tcstof(CString(value), NULL);
						bUdatePtn = TRUE;
						break;
					case PROP_WORK_2ND_WIDTH:
						pJob->m_d2NdWidth = _tcstof(CString(value), NULL);
						bUdatePtn = TRUE;
						break;
					case PROP_WORK_2ND_OVERLEN:
						pJob->m_d2NdOver = _tcstof(CString(value), NULL);
						bUdatePtn = TRUE;
						break;
					case PROP_WORK_PEN_POWER:
						(*pJob->m_pPens)[pJob->m_iPenID].m_dblPower = _tcstof(CString(value), NULL);
						break;
					case PROP_WORK_PEN_JUMPSPD:
						(*pJob->m_pPens)[pJob->m_iPenID].m_dblJumpSpeed = _tcstof(CString(value), NULL);
						break;
					case PROP_WORK_PEN_WELDSPD:
						(*pJob->m_pPens)[pJob->m_iPenID].m_dblMarkSpeed = _tcstof(CString(value), NULL);
						break;
					}

					if (bUdatePtn)
					{
						m_pDoc->DrawUpdateJob(pJob);
					}
	...
	if (m_pJob)
	{
		...
		{
			m_pJob->GetEntity()->UpdateEntity(dwData, value);
		}
		else
		{
			...
			// sjyi 2025.01.27 스파이럴 패턴 개별 속성 변경되도록 수정
			if (m_pJob->GetEntity()->GetEntityType() == LC_ENT_SPIRAL)
			{
				m_pJob->GetEntity()->UpdateEntity(dwData, value);
			}
			...
			if (bUpdate)
			{
				// sjyi 2023.04.14 헤어핀 패턴 회전 내용 추가
