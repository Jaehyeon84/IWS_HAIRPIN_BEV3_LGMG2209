...
void CDlgVScanCalib_Scan::InitDlgData()
{
	...
	float fXmm = (100.0f - m_pDlgCalib->m_iOverlap) * 0.01f* g_pGrabImgDlg->m_nGrabWd*resolX_umPcnt*0.001f;
	float fYmm = (100.0f - m_pDlgCalib->m_iOverlap) * 0.01f* g_pGrabImgDlg->m_nGrabHt*resolX_umPcnt*0.001f;
...
{
	unsigned char *pSnap = g_pGrabImgDlg->SnapImg();
	if (m_pSnapImg != NULL)
	{
		imaqDispose(m_pSnapImg);
		m_pSnapImg = NULL;
	...
	if (imaqArrayToImage(m_pSnapImg, pSnap,
		g_pGrabImgDlg->m_nGrabWd, g_pGrabImgDlg->m_nGrabHt) == 0)
	{
...
void CDlgVScanCalib_Scan::OnEnChangeEditScanOverlapP()
{
	...
	float fXmm = (100.0f - overlap) * 0.01f* g_pGrabImgDlg->m_nGrabWd*fResolution_umPpx*0.001f;
	float fYmm = (100.0f - overlap) * 0.01f* g_pGrabImgDlg->m_nGrabHt*fResolution_umPpx*0.001f;
...
void CDlgVScanCalib_Scan::OnBnClickedButtonDoScan()
{
	...
	g_pGrabImgDlg->m_pScanImgsProc->UseDisableScanLimit(TRUE);
	g_pGrabImgDlg->ChangeOverlap(m_pDlgCalib->m_iOverlap, m_pDlgCalib->m_iOverlap);
	g_pGrabImgDlg->ChangeScanRange(resolX_umPcnt, resolY_umPcnt, m_pDlgCalib->m_dScanXmin, m_pDlgCalib->m_dScanYmin, m_pDlgCalib->m_dScanXmax, m_pDlgCalib->m_dScanYmax);
...
#pragma omp parallel for 
	...
	{
		CScanImgsProc* pImgProc = g_pGrabImgDlg->m_pScanImgsProc;
