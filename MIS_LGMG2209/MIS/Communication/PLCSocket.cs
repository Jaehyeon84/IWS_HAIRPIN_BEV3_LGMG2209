...
namespace MIS.Communication
{
    class PLCSocket
    {
        ...
        protected int m_nStopType = 1;
        protected int m_nStopReason = 1;
        ...
        protected bool ReadPLCRequstData()
        {
            ...
            inputSignal[(int)INPUT_FRAME.STOP_SET] = GetBitToUshort(bitArray[1]);     //StopSet Request
            inputSignal[(int)INPUT_FRAME.STOP_RESET] = GetBitToUshort(bitArray[2]);     //StopReset Request
            ...
            // Stop Type
            m_nStopType = BitConverter.ToInt16(rcvData, 44);

            // Stop Reason
            m_nStopReason = BitConverter.ToInt16(rcvData, 46);
        ...
        protected bool WritePLCResponseData()
        {
            ...
            m_MelsecSock.Write(m_cConfig.PLC_Area, int.Parse(m_cConfig.PLC_WriteStatus), sndData.Length/2, sndData);

            return true;
        }

        // PC -> PLC 비트 값을 byte 배열로 변경
        protected byte[] GetOutputResponse()
        {
            writePlcList.Clear();

            ushort tmpWORD1 = Convert.ToUInt16(string.Format("0000000000000{2}{1}{0}", 
                outputSignal[(int)OUTPUT_FRAME.LINK],
                outputSignal[(int)OUTPUT_FRAME.STOP_SET],
                outputSignal[(int)OUTPUT_FRAME.STOP_RESET]));
        ...
        // Stop Set Req
        public bool GetStopSet()
        {
            return Convert.ToBoolean(inputSignal[(int)INPUT_FRAME.STOP_SET]);
        }

        // Stop Reset Req
        public bool GetStopReset()
        {
            return Convert.ToBoolean(inputSignal[(int)INPUT_FRAME.STOP_RESET]);
        }
        ...
        public bool GetStopSetRes()
        {
            return Convert.ToBoolean(outputSignal[(int)OUTPUT_FRAME.STOP_SET]);
        }

        public bool GetStopResetRes()
        {
            return Convert.ToBoolean(outputSignal[(int)OUTPUT_FRAME.STOP_RESET]);
        }
        ...
        public void SetStopSet(bool isFlag)
        {
            outputSignal[(int)OUTPUT_FRAME.STOP_SET] = GetBitToUshort(isFlag);

            // Logging
            if (isFlag == true)
            {
                LogManager.WriteLog(LogType.Information, "[PC->PLC] Equip Stop Set Response ON");
            }
            else
            {
                LogManager.WriteLog(LogType.Information, "[PC->PLC] Equip Stop Set Response OFF");
            }
        }

        public void SetStopReset(bool isFlag)
        {
            outputSignal[(int)OUTPUT_FRAME.STOP_RESET] = GetBitToUshort(isFlag);

            // Logging
            if (isFlag == true)
            {
                LogManager.WriteLog(LogType.Information, "[PC->PLC] Equip Stop Reset Response ON");
            }
            else
            {
                LogManager.WriteLog(LogType.Information, "[PC->PLC] Equip Stop Reset Response OFF");
            }
        }
        ...
        {
            if(m_cConfig.m_bJudgeByPLC) // PLC 판정 사용
            {
                return strFinalJudgement_A;
            }

            // sjyi 2024.05.17 개별 판정값 확인하여 OK, NG 판정하도록 수정
            string strFinalJudgement = string.Format("OK");

            for (int i=0; i< lstGMESInsptDataAJig.Count(); i++)
            {
                int nAftCnt = lstGMESInsptDataAJig[i].LsJudgement.Count();

                if(nAftCnt <= 0)
                {
                    strFinalJudgement = "NG";

                    return strFinalJudgement;
                }

                if (lstGMESInsptDataAJig[i].LsJudgement[nAftCnt - 1] == "NG")
                {
                    strFinalJudgement = "NG";
        ...
        {
            // 2024.05.30 PLC 판정 사용 여부 추가
            if (m_cConfig.m_bJudgeByPLC) //PLC값 참조 일경우, PLC값 그대로 리턴
            {
            ...
            for (int i = 0; i < lstGMESInsptDataBJig.Count(); i++)
            {
                //for (int j = 0; j < lstGMESInsptDataBJig[i].LsJudgement.Count(); j++)
                //{
                int nAftCnt = lstGMESInsptDataBJig[i].LsJudgement.Count();
                ...
                if (lstGMESInsptDataBJig[i].LsJudgement[nAftCnt - 1] == "NG")
                {
                    strFinalJudgement = "NG";
        ...
        // PLC->PC 검사 결과 확인 함수 ----------------------------------------------------
        // 전검사 결과 Update
        public void ReadBefInspect(int nJibNo, bool bFist, bool bRepair = false)
        {
            ...
            while (bReadOk == false)
            {
                try
                {
		            ...
		            {
                        nPLCAddressId = nPLCAddress + i * tmpWord;
                        tmpbytes = m_MelsecSock.Read(m_cConfig.PLC_Area, nPLCAddressId, tmpWord);
                        LogManager.WriteLog(LogType.Information, string.Format("[READ Bef Insp]-{0} addr : {1}", i + 1, nPLCAddressId));
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        LogManager.WriteLog(LogType.Information, "GetInspection Data Read Error - " + ex.ToString());
                        bReadOk = true;
                    }
            ...
            if (bRepair == true)
            {
                for(int i = 0; i< LsPlcBefInsptDataWORDS.Count; i++)
                {
                    LsPlcBefInsptDataWORDS[i] = 0;
                }
            }
        ...
        protected bool ConverAfterInspectWordToGMESData(List<short> lstRawData, List<InspDataGMES> lstInsptData)
        {
            ...
            for (int i = 0; i < lstRawData.Count(); i++)
            {
                ...
                {
                    if(lstRawData[i] == (short)eJudgement.RW_OK)
                    {
                        lstInsptData[i].LsJudgement.Add(eJudgement.RW_OK.ToString());
                    }
                    else if(lstRawData[i] == (short)eJudgement.RW_NG)
                    {
                        lstInsptData[i].LsJudgement.Add(eJudgement.RW_NG.ToString());
                    }
                }
            }
        ...
        public void ClearInspecData(int nJigNo)
        {
            List <InspDataGMES> listInspData = GetInspectData(nJigNo);
            ...
            for (int i = 0; i < m_cConfig.Slot; i++)
            {
                for (int j = 0; j < (m_cConfig.Layer / 2); j++)
                {
                    ...
                    listInspData.Add(tmp);
                }
            }

            for(int i=0; i<listInspData.Count(); i++)
            {
                listInspData[i].LsJudgement.Clear();
            }

            // Logging
            string strLog = string.Format("!!! Clear Inspect Buffer !!! JigNo: {0}, listInspData: {1}, LsJudgement: {2}",
                nJigNo,
                listInspData.Count(),
                listInspData[0].LsJudgement.Count());
            LogManager.WriteLog(LogType.Information,  strLog);
        }

        public void ReadExtInfo(int nJigNo)
        {
            // Check Connected Status
            if (m_MelsecSock.IsConnected == false)
            {
            ...
            // PLC Read Start(Logging)
            LogManager.WriteLog(LogType.Information, "Get Ext Info Start");

            // PLC Read Address
            int nPLCAddress = 0;
            if (nJigNo == 0) // Jig A
            {
                nPLCAddress = int.Parse(m_cConfig.PLC_ReadExtInfo_A);
            }
            else // Jig B
            {
                nPLCAddress = int.Parse(m_cConfig.PLC_ReadExtInfo_B);
            }

            // Read From PLC
            byte[] rcvData = m_MelsecSock.Read(m_cConfig.PLC_Area, nPLCAddress, 20);

            extInfo.LoadRate_HeadX = BitConverter.ToInt16(rcvData, 0);
            extInfo.LoadRate_HeadZ = BitConverter.ToInt16(rcvData, 2);
            extInfo.LoadRate_ClampY = BitConverter.ToInt16(rcvData, 4);
            extInfo.LoadRate_ClampZ = BitConverter.ToInt16(rcvData, 6);
            extInfo.LoadRate_ClampC = BitConverter.ToInt16(rcvData, 8);
            extInfo.LoadRate_CenteringZ = BitConverter.ToInt16(rcvData, 10);

            // PLC Read Start(Logging)
            LogManager.WriteLog(LogType.Information, "Get Ext Info End");
