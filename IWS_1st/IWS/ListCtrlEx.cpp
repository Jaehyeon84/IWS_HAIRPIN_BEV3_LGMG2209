...
// 06/05/12 :
//		Add bool m_bDisplayEditorCalled;				//Add by MaxMax14 le 06/05/12 pour d?ecter les modifications
//		Add void SetDisplayEditorFlag (bool bEtat);		//Add by MaxMax14 le 06/05/12: mise ?true/false de m_bDisplayEditorCalled
//		Add BOOL GetDisplayEditorFlag ();				//Add by MaxMax14 le 06/05/12: Renvoi ?at (true/false) de m_bDisplayEditorCalled
//
// 05/10/13 :
...
:m_clrBack(-1)
,m_clrText(-1)
,m_clrBackUnfocus(-1)	//Add MaxMax14 30/09/13
,m_clrTextUnfocus(-1)	//Add MaxMax14 30/09/13
,m_dwUserData(NULL)
,m_nColumn(nColumn)
{
...
//,m_clrBackUnfocus(::GetSysColor(COLOR_HIGHLIGHT))
,m_clrBackUnfocus(0xFFFFFFFF)	//Add MaxMax14 30/09/13
//,m_clrTextUnfocus(::GetSysColor(COLOR_HIGHLIGHTTEXT))
,m_clrTextUnfocus(0xFFFFFFFF)	//Add MaxMax14 30/09/13
,m_dwUserData(dwUserData)
{
}
...
CListCtrlEx::ItemData::ItemData(COLORREF clrBack, COLORREF clrText, COLORREF clrBackUnfocus, COLORREF clrTextUnfocus, DWORD_PTR dwUserData /*= NULL*/)//Add by MaxMax14 30/09/13
:m_clrBack(clrBack)
,m_clrText(clrText)
,m_clrBackUnfocus(clrBackUnfocus)	//Add by MaxMax14 30/09/13
,m_clrTextUnfocus(clrTextUnfocus)	//Add by MaxMax14 30/09/13
,m_dwUserData(dwUserData)
{
}
...
,m_bDisplayEditorCalled(FALSE)		//Add by MaxMax14 le 06/05/12
,m_bShowSelectedItemUnFocus(FALSE)	//Add by MaxMax14 le 05/10/13
{
#ifndef _WIN32_WCE
...
int CALLBACK CListCtrlEx::CompareProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	if(nLeft < 0) nLeft = (int)lParam1;
	if(nRight < 0) nRight = (int)lParam2;
	int nCount = This->GetItemCount();
...
BOOL CListCtrlEx::DisplayEditor(int nItem, int nSubItem)
{
	...
	m_bDisplayEditorCalled = TRUE;	//Add by MaxMax14 le 06/05/12
...
//DWORD dwdata = NULL;
	...
	switch(lplvcd->nmcd.dwDrawStage)
	{
        ...
        case CDDS_ITEMPOSTPAINT:	//Add by MaxMax14
			{
				COLORREF clrBack = m_clrDefBackUnfocus;
				COLORREF clrText = m_clrDefTextUnfocus;
				...
				if ( m_bShowSelectedItemUnFocus )
				{
					...
					{
					//DEBUT du Fond -----------------------------------------------
						//http://www.codersource.net/mfc/mfcclistbox/clistboxchangingfontandcolors.aspx
						// Draw border
						//http://stackoverflow.com/questions/12936312/can-someone-explain-me-the-cdcselectobject-to-me
						...
						if(pData)
						{
							clrBack = pData->m_clrBackUnfocus;
							clrText = pData->m_clrTextUnfocus;
							if(clrBack == 0xFFFFFFFF && clrText == 0xFFFFFFFF)
							{
								clrBack = m_clrDefBackUnfocus;
								clrText = m_clrDefTextUnfocus;
							}
						}
						CBrush brush (clrBack);

						CBrush * pOldBrush = dc-> SelectObject (& brush);
						rect.left = rect.left+2;		//de Alessandro Di Martino	20-Nov-15 19:20 //http://www.codeproject.com/Tips/670172/Get-Highlight-Selected-Items-on?msg=5163903#xx5163903xx
						dc->FillRect(&rect, &brush);	//Rempli un rectangle
					//FIN du Fond -----------------------------------------------

					//DEBUT 1 Ecriture du text----------------------------------------
					//	de http://www.tutorialsto.com/software/c/clistctrl-landscaping-shown-in-the-focus-of-bank.html
					// Draw the focus of the beginning of each line of text out
						//CFont * pOldFont = dc-> SelectObject (pFont);
						CString strTemp;
						int nColumns = this->GetColumnCount();	//GetColumns();
						HDITEM hditem;
						UINT nFormat;
						int nFmt;
						...
						for (int i = 0; i <nColumns; i++)
						{
							...
							rect.DeflateRect(6, 0); // shrink our rect x pixels in each direction : Pour corriger l'alignment du texte ?gauche MaxMax14
							hditem.mask = HDI_FORMAT;	// Get each word out of alignment
							...
							dc-> SelectStockObject (NULL_BRUSH); // set NULL_BRUSH
							CFont * pOldFont = dc-> SelectObject (pFont);
							int iBkMode = dc->SetBkMode(TRANSPARENT);
							dc->SetTextColor(clrText);
							dc-> DrawText (strTemp, & rect, nFormat);
						}
					//FIN 1 Ecriture du text----------------------------------------

					//DEBUT Dessine la bordure ---------------------------------
						CPen m_SelPen;                                  //declare a new Pen object
						if (GetItemState(iRow, LVIS_FOCUSED) == LVIS_FOCUSED)
						{
							m_SelPen.CreatePen(m_iPenStyles, 1, RGB(255, 0, 0));  //Create the GDI Pen, dashed, x pixels wide, black. PS_SOLID PS_DASH PS_DOT
						}
						else
						{
							m_SelPen.CreatePen(m_iPenStyles, 1, m_clrPenStyle);  //Create the GDI Pen, dashed, x pixels wide, black. PS_SOLID PS_DASH PS_DOT
						}
						
						CPen * pOldPen = dc-> SelectObject (&m_SelPen); //Tell the DC to use this pen from now on.
						GetItemRect (iRow, rect, LVIR_BOUNDS);
						dc-> SelectStockObject (NULL_BRUSH); // set NULL_BRUSH
						if ( m_bBordure )
						{	  
							rect.DeflateRect(2, 1); // shrink our rect x pixels in each direction
							dc-> Rectangle (rect);
						}
					//FIN Dessine la bordure ---------------------------------
					//On remet les anciens param?res	// put back the old objects
						dc-> SelectObject (pOldPen);
						dc-> SelectObject (pOldBrush);
						dc-> SelectObject (pFont);
						DeleteObject(m_SelPen);
						DeleteObject(brush);
						ReleaseDC(dc);
					}
				}
...
void CListCtrlEx::SetUnFocusColors( COLORREF clrBackUnfocus, COLORREF clrTextUnfocus)	//MaxMax14 05/10/13
{
	m_clrDefTextUnfocus = clrTextUnfocus;	
	m_clrDefBackUnfocus = clrBackUnfocus;	
}

void CListCtrlEx::SetRowColors(int nItem, COLORREF clrBk, COLORREF clrText, COLORREF clrBackUnfocus, COLORREF clrTextUnfocus)	//MaxMax14 05/10/13
{
	ItemData* pData = (ItemData*)GetItemDataInternal(nItem);
	...
	pData->m_clrTextUnfocus = clrTextUnfocus;
	pData->m_clrBackUnfocus = clrBackUnfocus;
	Update(nItem);
}

void CListCtrlEx::SetRowColorsUnfocus(int nItem, COLORREF clrBackUnfocus, COLORREF clrTextUnfocus)	//MaxMax14 05/10/13
{
	ItemData* pData = (ItemData*)GetItemDataInternal(nItem);
	if(!pData) SetItemData(nItem, 0);
	pData = (ItemData*)GetItemDataInternal(nItem);
	...
	pData->m_clrBackUnfocus = clrBackUnfocus;
//MaxMax14 05/10/13
	if (pData->m_clrText == -1)
	{
		pData->m_clrText = m_clrDefText;	
	}
	if (pData->m_clrBack == -1)
	{
		pData->m_clrBack = m_clrDefBack;
	}
...
void CListCtrlEx::SetRowColors(int nItem, COLORREF clrBk, COLORREF clrText)
{
	...
	if (pData->m_clrTextUnfocus == -1)
	{
		pData->m_clrTextUnfocus = m_clrDefTextUnfocus;	//MaxMax14 05/10/13
	}
	if (pData->m_clrBackUnfocus == -1)
	{
		pData->m_clrBackUnfocus = m_clrDefBackUnfocus;	//MaxMax14 05/10/13
	}
...
{
	m_clrDefBack = RGB(255, 255, 255);// GetTextBkColor();	// Retrieves background color of text labels in the control.
	m_clrDefText = RGB(0, 0, 0);// GetTextColor();		// Retrieves foreground color of text labels in the control.
	
	//Add by MaxMax14 30/09/13
	m_clrDefBackUnfocus = ::GetSysColor(COLOR_HIGHLIGHT);		//COLOR_HIGHLIGHT COLOR_HIGHLIGHTTEXT	//http://msdn.microsoft.com/en-us/library/windows/desktop/ms724371%28v=vs.85%29.aspx	
	m_clrDefTextUnfocus = ::GetSysColor(COLOR_HIGHLIGHTTEXT);	//COLOR_HIGHLIGHT COLOR_HIGHLIGHTTEXT	//http://msdn.microsoft.com/en-us/library/windows/desktop/ms724371%28v=vs.85%29.aspx

	m_iPenStyles = PS_DOT;		//ex: PS_SOLID PS_DASH PS_DOT
	m_clrPenStyle = RGB(255, 255, 255);
	m_bBordure = FALSE;
...
int CListCtrlEx::Compare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	CString strLeft = This->GetItemText((int)lParam1, nSubItem);
	CString strRight = This->GetItemText((int)lParam2, nSubItem);
...
{
	return (int)(_tstoi(pLeftText) - _tstoi(pRightText));
}
...
{
	LONGLONG l1 = _tstol(pLeftText);
	LONGLONG l2 = _tstol(pRightText);
	if(l1 && l2 && (l1 - l2))
	{
		CString str1, str2;
		str1.Format(L"%ld", l1);
...
//-------------------------------------------------------------
void CListCtrlEx::SetDisplayEditorFlag (BOOL bEtat)	//Add by MaxMax14 le 06/05/12: mise ?true/false de m_bDisplayEditorCalled
{
	m_bDisplayEditorCalled = bEtat;
}

//-------------------------------------------------------------
BOOL CListCtrlEx::GetDisplayEditorFlag ()	//Add by MaxMax14 le 06/05/12: Renvoi ?at (true/false) de m_bDisplayEditorCalled
{
	return m_bDisplayEditorCalled;
}

//-------------------------------------------------------------
void CListCtrlEx::SetShowSelectedItemUnFocus (BOOL bEtat)	//True: Show selected item without focus //Add by MaxMax14 le 05/10/13
{
	m_bShowSelectedItemUnFocus = bEtat;
}

//-------------------------------------------------------------
BOOL CListCtrlEx::GetShowSelectedItemUnFocus ()	//True: Show selected item without focus //Add by MaxMax14 le 05/10/13
{
	return m_bShowSelectedItemUnFocus;
}

//-------------------------------------------------------------
void CListCtrlEx::SetPenStyle (int iStyle)	//D?inie type de trait //ex: PS_SOLID PS_DASH PS_DOT //Add by MaxMax14 le 05/10/13
{
	m_iPenStyles = iStyle;
}

//-------------------------------------------------------------
void CListCtrlEx::SetPenStyleColor (COLORREF clrPenStyle) //Add by MaxMax14 le 05/10/13
{
	m_clrPenStyle = clrPenStyle;	//RGB(255, 255, 255); //white
}

//-------------------------------------------------------------
void CListCtrlEx::SetBordureActive (BOOL bBordure)	//D?inie si on dessine la bordure //Add by MaxMax14 le 05/10/13
{
	m_bBordure = bBordure;
}
