...
void CDlgMatrixPCC::OnBnClickedButtonApply()
{
	...
	case 1:
		...
		for (int i = 0; i < m_pDoc->m_config.m_nMatrixDiv; i++) {
			m_pDoc->m_config.m_pMatrixA[i].a11 = m_dMatrix[0];
			m_pDoc->m_config.m_pMatrixA[i].a12 = m_dMatrix[1];
			m_pDoc->m_config.m_pMatrixA[i].a13 = m_dMatrix[2];
			m_pDoc->m_config.m_pMatrixA[i].a21 = m_dMatrix[3];
			m_pDoc->m_config.m_pMatrixA[i].a22 = m_dMatrix[4];
			m_pDoc->m_config.m_pMatrixA[i].a23 = m_dMatrix[5];
		}
		break;
	case 2:
		//for (int i = 0; i < MATRIX_DIVISION; i++) {
		for (int i = 0; i < m_pDoc->m_config.m_nMatrixDiv; i++) {
			m_pDoc->m_config.m_pMatrixB[i].a11 = m_dMatrix[0];
			m_pDoc->m_config.m_pMatrixB[i].a12 = m_dMatrix[1];
			m_pDoc->m_config.m_pMatrixB[i].a13 = m_dMatrix[2];
			m_pDoc->m_config.m_pMatrixB[i].a21 = m_dMatrix[3];
			m_pDoc->m_config.m_pMatrixB[i].a22 = m_dMatrix[4];
			m_pDoc->m_config.m_pMatrixB[i].a23 = m_dMatrix[5];
		}
		break;
	default:
		//for (int i = 0; i < MATRIX_DIVISION; i++) {
