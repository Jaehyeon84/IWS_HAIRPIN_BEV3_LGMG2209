...
CButtonST::CButtonST()
{
	...
	m_crColors[BTNST_COLOR_BK_IN] = COLOR_GREY28;
	m_crColors[BTNST_COLOR_FG_IN] = COLOR_OLIVEDRAB1;
	m_crColors[BTNST_COLOR_BK_OUT] = COLOR_GREY26;
	m_crColors[BTNST_COLOR_FG_OUT] = COLOR_GREY100;
	m_crColors[BTNST_COLOR_BK_FOCUS] = COLOR_GREY28;
	m_crColors[BTNST_COLOR_FG_FOCUS] = COLOR_GREY100;
...
//	CBrush		brBackground(crColor);
 	CPen penBorder;
 	penBorder.CreatePen(PS_SOLID, 1, COLOR_GREY41);
 	CPen* pOldPen = pDC->SelectObject(&penBorder);
	CBrush* pOldBrush = (CBrush*)pDC->SelectStockObject(NULL_BRUSH);
	pDC->RoundRect(pRect, CPoint(6, 6));
...
DWORD CButtonST::OnDrawBorder(CDC* pDC, CRect* pRect)
{
	if (m_bIsPressed){
		CPen penBorder;
		penBorder.CreatePen(PS_SOLID, 1, COLOR_OLIVEDRAB1);
		CPen* pOldPen = pDC->SelectObject(&penBorder);
		pDC->RoundRect(pRect, CPoint(6, 6));
		//pDC->FillRect(pRect, &brBackground);
		pDC->SelectObject(pOldPen);
