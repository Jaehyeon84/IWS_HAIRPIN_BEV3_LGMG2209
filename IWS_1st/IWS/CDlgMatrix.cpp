	...
	: CBaseDialog(IDD_DIALOG_MATRIX, pParent)
{
	...
	{
		m_dOnebyOneMatchingX[i] = 1.f;
		m_dOnebyOneMatchingY[i] = 1.f;
	}
}
...
CDlgMatrix::~CDlgMatrix()
{
	...
	//for (int i = 0; i < m_pDoc->m_config.m_nMatrixDiv; ++i)
	//{
	//	delete[] m_pdMatrixDiv[i];  // 각 행에 할당된 열 메모리 해제
	//}
	//delete[] m_pdMatrixDiv;  // 행 포인터 배열 해제
...
BOOL CDlgMatrix::OnInitDialog()
{
	...
	// Set Matrix Config
	//m_pdMatrixDiv = new double*[m_pDoc->m_config.m_nMatrixDiv];
	//for (int i = 0; i < m_pDoc->m_config.m_nMatrixDiv; i++) 
	//{
	//	m_pdMatrixDiv[i] = new double[6];
	//}
...
void CDlgMatrix::OnBnClickedButtonPLCUpdate()
{
	...
	int nModelNo = m_pDoc->m_plc.GetModelNo();

	SetDlgItemInt(IDC_STATIC_PLC_MODELNO, nModelNo);
...
void CDlgMatrix::OnBnClickedButtonCalc()
{
	...
	for (auto p : m_pDoc->m_Vec3DData)
	for(long idx = 0 ; idx < (long)m_pDoc->m_Vec3DData.size() ; idx++)
	{
		auto pt3D = m_pDoc->m_Vec3DData[idx];
		auto ptIWS = m_pDoc->m_VecIWSData[idx];

		double x = ptIWS.dX / pt3D.dX;
		double y = ptIWS.dY / pt3D.dY;

		m_dOnebyOneMatchingX[idx] = x;
		m_dOnebyOneMatchingY[idx] = y;
	...
	for (int i = 0; i < m_pDoc->m_config.m_nMatrixDiv; i++)
	{
		...
		m_pdMatrixDiv[i][0] = m_dMatrix[0];
		m_pdMatrixDiv[i][1] = m_dMatrix[1];
		m_pdMatrixDiv[i][2] = m_dMatrix[2];
		m_pdMatrixDiv[i][3] = m_dMatrix[3];
		m_pdMatrixDiv[i][4] = m_dMatrix[4];
		m_pdMatrixDiv[i][5] = m_dMatrix[5];
...
{	
	...
	int nsize = m_pDoc->m_VecIWSData.size() / m_pDoc->m_config.m_nMatrixDiv;
	int n = 0;
...
void CDlgMatrix::OnBnClickedButtonApply()
{
	...
	case 1:
		...
		for (int i = 0; i < m_pDoc->m_config.m_nMatrixDiv; i++) {
			...
			m_pDoc->m_config.m_pMatrixA[i].a11 = m_pdMatrixDiv[i][0];
			m_pDoc->m_config.m_pMatrixA[i].a12 = m_pdMatrixDiv[i][1];
			m_pDoc->m_config.m_pMatrixA[i].a13 = m_pdMatrixDiv[i][2];
			m_pDoc->m_config.m_pMatrixA[i].a21 = m_pdMatrixDiv[i][3];
			m_pDoc->m_config.m_pMatrixA[i].a22 = m_pdMatrixDiv[i][4];
			m_pDoc->m_config.m_pMatrixA[i].a23 = m_pdMatrixDiv[i][5];
			...
			strMsg.Format(L"MATRIX %02d [%.6f][%.6f][%.6f][%.6f][%.6f][%.6f]", i + 1, m_pdMatrixDiv[i][0], m_pdMatrixDiv[i][1], m_pdMatrixDiv[i][2], m_pdMatrixDiv[i][3], m_pdMatrixDiv[i][4], m_pdMatrixDiv[i][5]);
		...
		for(int i = 0; i < MAX_HAIRPIN_SLOT * MAX_HAIRPIN_LAYER; i++)
		{
			m_pDoc->m_config.m_dOnebyOneMatchingX_JigA[i] = m_dOnebyOneMatchingX[i];
			m_pDoc->m_config.m_dOnebyOneMatchingY_JigA[i] = m_dOnebyOneMatchingY[i];
		}
		break;
	case 2:
		//for (int i = 0; i < MATRIX_DIVISION; i++) {
		for (int i = 0; i < m_pDoc->m_config.m_nMatrixDiv; i++) {
			...
			m_pDoc->m_config.m_pMatrixB[i].a11 = m_pdMatrixDiv[i][0];
			m_pDoc->m_config.m_pMatrixB[i].a12 = m_pdMatrixDiv[i][1];
			m_pDoc->m_config.m_pMatrixB[i].a13 = m_pdMatrixDiv[i][2];
			m_pDoc->m_config.m_pMatrixB[i].a21 = m_pdMatrixDiv[i][3];
			m_pDoc->m_config.m_pMatrixB[i].a22 = m_pdMatrixDiv[i][4];
			m_pDoc->m_config.m_pMatrixB[i].a23 = m_pdMatrixDiv[i][5];
		...
		for(int i = 0; i < MAX_HAIRPIN_SLOT * MAX_HAIRPIN_LAYER; i++)
		{
			m_pDoc->m_config.m_dOnebyOneMatchingX_JigB[i] = m_dOnebyOneMatchingX[i];
			m_pDoc->m_config.m_dOnebyOneMatchingY_JigB[i] = m_dOnebyOneMatchingY[i];
		}
		break;
	default:
		//for (int i = 0; i < MATRIX_DIVISION; i++) {
	...
	if (m_pDoc->m_config.m_bMatrixByModel == TRUE)
	{
		m_pDoc->m_config.WriteMatrix(m_pDoc->m_plc.GetModelNo());
	}
	else
	{
...
void CDlgMatrix::OnBnClickedButtonSave()
{
	...
	FileSaveFitData(); // 2023.06.07 좌표 매칭 결과 데이터 저장용
}
...
void CDlgMatrix::FileSaveFitData()
{
	TCHAR path[_MAX_PATH];
	GetModuleFileName(NULL, path, sizeof path);
...
//	strStartUpPath = strStartUpPath + L"\\MatrixData.csv";
	...
	{
	case 1: // Jig A
		strStartUpPath = strStartUpPath + L"\\MatrixResultData_JigA.csv";
		break;
	case 2: // Jig B
		strStartUpPath = strStartUpPath + L"\\MatrixResultData_JigB.csv";
		break;
	default:	// Unknown
		strStartUpPath = strStartUpPath + L"\\MatrixResultData.csv";
		break;
	}
	// sjyi 2022-07-08 JigA, JigB를 구분하기 위해, 수정 <--
...
//		nFitDataIndex = (nLayer - 1) * 96 + (nSlot - 1); // Layer별
		nFitDataIndex = i;	// 4분할

		if((long)m_pDoc->m_VecFitdata.size() > 0)
		{
			pt1.dX = m_pDoc->m_VecFitdata[nFitDataIndex].dX;
			pt1.dY = m_pDoc->m_VecFitdata[nFitDataIndex].dY;
		}
		else
		{
			pt1.dX = 0.f;
...
//	int loopCount = (int)(m_pDoc->m_Vec3DData.size() / MATRIX_DIVISION);
	int loopCount = (int)(m_pDoc->m_Vec3DData.size() / m_pDoc->m_config.m_nMatrixDiv);
	...
	double arr_xl[6] = { 0.5,	-0.5,	-10.0,	-0.5,	0.5,	-10 };
	double arr_xu[6] = { 1.5,	0.5,	10.0,	0.5,	1.5,	10 };
	double epsx = 1e-64;
...
void CDlgMatrix::OnBnClickedBtnLoad()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.

	CString strFilter;
	strFilter = _T("Position File(*.csv)|*.csv||");
	// 프로그램 실행 경로
	TCHAR path[_MAX_PATH];
	GetModuleFileName(NULL, path, sizeof path);
	...
	strStartUpPath = dlgFile.GetPathName();

	FileLoad(strStartUpPath);

	DrawChart();
	
}

void CDlgMatrix::FileLoad(CString strPath)
{
	CFile cfile;
	if (!cfile.Open(strPath, CFile::modeRead))
	{
		_tracer(TRACE_ERROR, 1, _T("File Load Fail Matrix Data."));
		return;
	}
	...
	// Read Header
	ar.ReadString(szBufferData, 200);
	...
	int i = 0;
	while (ar.ReadString(szBufferData, 200))
	{
		ParsePositionInfo(szBufferData, i);
		i++;
	}
...
void CDlgMatrix::ParsePositionInfo(TCHAR* szCSV, int nIndex)
{
	...
	if (strData.size() < 6) return;

	IWS_LOCATION pt;

	// 3DData
	pt.dX = _tstof(strData[2]);
	pt.dY = _tstof(strData[3]);
	m_pDoc->m_Vec3DData.push_back(pt);

	// IWSData
	pt.dX = _tstof(strData[4]);
	pt.dY = _tstof(strData[5]);
	m_pDoc->m_VecIWSData.push_back(pt);
}

void CDlgMatrix::Tokenize(std::vector<CString> &VecStr, CString strSource, TCHAR tToken)
{
