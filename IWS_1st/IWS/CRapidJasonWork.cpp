...
void CRapidJasonWork::MakeJasonHeaderHairpin(RJsonWriter& writer, CIWSDoc *pDoc)
{
	...
	// sjyi 2025.09.10 Sortie Rpt Map 추가
	writer.Key("m_nSortieRptMap");
	writer.StartArray();        // "["Sortie Map 시작
	for (int i = 0; i < MAX_NUM_REPEAT_WELD; i++)
	{
		writer.StartArray();    // "[" N번째 요소 시작
		writer.Int(pDoc->m_work.m_hp.m_nSortieRptMap[i]);
		writer.EndArray();      // "]" N번째 요소 끝
	}
	writer.EndArray();          // "]"Sortie Map 끝
...
BOOL CRapidJasonWork::ParseJsonHeaderHairpin(RJsonValue& reader, CIWSDoc *pDoc)
{
	...
	// sjyi 2025.09.10 Sortie Rpt Map 추가
	RJsonValue& ValArraySortieMap = valHeader["m_nSortieRptMap"];
	for (int i = 0; i < ValArraySortieMap.Size(); i++)
	{
		RJsonValue& ValData = ValArraySortieMap[i];
		pDoc->m_work.m_hp.m_nSortieRptMap[i] = GetJsInt(0, ValData, 0);
	}
