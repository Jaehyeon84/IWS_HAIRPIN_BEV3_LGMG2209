...
#include "EntitySpiral.h" // sjyi 2024.02.01 Spiral 추가
...
CIWSWorkHairpin::CIWSWorkHairpin()
{
	...
	m_n2DPathLength = 0;
	m_str2DRecipePath = _T("");
	m_b2DInspection = FALSE;
...
void CIWSWorkHairpin::SaveWork(CFile * pf, CIWSDoc * pDoc)
{
	...
	pf->Write(&m_n2DPathLength, sizeof(int));
	pf->Write(m_str2DRecipePath, sizeof(TCHAR)*m_n2DPathLength);
	pf->Write(&m_b2DInspection, sizeof(BOOL));
...
BOOL CIWSWorkHairpin::LoadWork(CFile * pf, CIWSDoc * pDoc)
{
	...
	case WORK_VER104:
		...
		break;
	case WORK_VER105:
		ReadHairpinInfoV105(pf);
		pf->Read(&m_zComp, sizeof(IWS_HAIRPIN_Z_COMP));
		break;
	case WORK_VER106: // v106
		ReadHairpinInfoV106(pf);
		pf->Read(&m_zComp, sizeof(IWS_HAIRPIN_Z_COMP));
		break;
	case WORK_VER107:
		ReadHairpinInfoV106(pf);
		...
		break;
	case WORK_VER108:
		ReadHairpinInfoV108(pf);
		pf->Read(&m_zComp, sizeof(IWS_HAIRPIN_Z_COMP));
		break;
	case WORK_VER109:
		ReadHairpinInfoV109(pf);
		pf->Read(&m_zComp, sizeof(IWS_HAIRPIN_Z_COMP));
		break;
	case WORK_VER110:
		ReadHairpinInfoV110(pf);
		pf->Read(&m_zComp, sizeof(IWS_HAIRPIN_Z_COMP));
		break;
	case WORK_VERC:	// 최신 버전
	...
	if (iFileVer > WORK_VER106)
	{
		m_n2DPathLength = 0;
		m_str2DRecipePath = _T("");
		m_b2DInspection = FALSE;

		pf->Read(&m_n2DPathLength, sizeof(int));
		
		TCHAR *szFileName = new TCHAR[m_n2DPathLength];
		pf->Read(szFileName, sizeof(TCHAR)*m_n2DPathLength);
		m_str2DRecipePath = (LPCTSTR)szFileName;
		
		//m_str2DRecipePath.Format(_T("%s"), szFileName);
		pf->Read(&m_b2DInspection, sizeof(BOOL));
...
void CIWSWorkHairpin::ReadHairpinInfoV103(CFile * pf)
{
	...
	// sjyi 2023.06.15 추가(Division 옵션 추가)
	SetDivParameters(m_hp.nWeldType, m_hp.nDivJumpOffset, m_hp.nDivStartSlot);

	// dhpark 2023.07.13 2D Offset 추가
	m_hp.b3DUse = TRUE;
	for (int i = 0; i < 512; i++)
	{
		m_hp.hpOffset2D[i].ax = 0;
		m_hp.hpOffset2D[i].ay = 0;
		m_hp.hpOffset2D[i].bx = 0;
		m_hp.hpOffset2D[i].by = 0;
	...
	// sjyi 2024.05.25 Sortie 반복, 스파이럴 패턴, 모래시계 패턴 추가
	m_hp.nGenDir = GEN_CCW;

	m_hp.bEnableSortiRpt = FALSE;
	m_hp.nSortieCnt = 0;
	m_hp.nSortieDir = SORTIE_ONEWAY;

	m_hp.HpSize2nd2 = 0;

	m_hp.bUseOutToInP1 = FALSE;
	m_hp.bUseOutToInP2 = FALSE;
	m_hp.bCloseSpiralP1 = TRUE;
	m_hp.bCloseSpiralP2 = TRUE;
}
...
// sjyi 2023.04.14 헤어핀 패턴 회전 각도 추가로 인해 Hairpin Info 구조체 버전업
void CIWSWorkHairpin::ReadHairpinInfoV104(CFile * pf)
{
	...
	for (int i = 0; i < MAX_NUM_REPEAT_WELD; i++)
	{
		m_hp.m_nSortieRptMap[i] = 0;
	}
}

void CIWSWorkHairpin::ReadHairpinInfoV105(CFile * pf)
{
	IWS_HAIRPIN_INFO_V105   hp;
	pf->Read(&hp, sizeof(IWS_HAIRPIN_INFO_V105));
...
// dhpark 2023.06.26 2D 검사 레시피 경로 변수 추가 인해 Hairpin Info 구조체 버전업
void CIWSWorkHairpin::ReadHairpinInfoV106(CFile * pf)
{
	IWS_HAIRPIN_INFO_V106   hp;
	pf->Read(&hp, sizeof(IWS_HAIRPIN_INFO_V106));
	...
	m_hp.dPtnRotationAngle = hp.dPtnRotationAngle;
	m_hp.dPtnRotationAngle2 = hp.dPtnRotationAngle2;

	//sjyi 2023.06.15 추가(Division 옵션 추가)
	m_hp.nWeldType = hp.nWeldType;
	m_hp.nDivJumpOffset = hp.nDivJumpOffset;
	m_hp.nDivStartSlot = hp.nDivStartSlot;
...
// sjyi 2024.02.28 헤어핀 외경(짝수열) 패턴 반전 변수 추가로 인해 Version Up
void CIWSWorkHairpin::ReadHairpinInfoV108(CFile * pf)
{
	IWS_HAIRPIN_INFO_V108   hp;
	pf->Read(&hp, sizeof(IWS_HAIRPIN_INFO_V108));
	...
	//// dhpark 2023.07.13 2D Offset 추가
	m_hp.b3DUse = hp.b3DUse;
	for (int i = 0; i < 512; i++)
	{
		m_hp.hpOffset2D[i].ax = hp.hpOffset2D[i].ax;
		m_hp.hpOffset2D[i].ay = hp.hpOffset2D[i].ay;
		m_hp.hpOffset2D[i].bx = hp.hpOffset2D[i].bx;
		m_hp.hpOffset2D[i].by = hp.hpOffset2D[i].by;
...
// sjyi 2024.02.28 헤어핀 외경(짝수열) 패턴 반전 변수 추가로 인해 Version Up
void CIWSWorkHairpin::ReadHairpinInfoV109(CFile * pf)
{
	IWS_HAIRPIN_INFO_V109   hp;
	pf->Read(&hp, sizeof(IWS_HAIRPIN_INFO_V109));
...
// sjyi 2024.10.17 Sortie 반복 맵 추가로 인해, Version Up
void CIWSWorkHairpin::ReadHairpinInfoV110(CFile * pf)
{
	IWS_HAIRPIN_INFO_V110   hp;
	pf->Read(&hp, sizeof(IWS_HAIRPIN_INFO_V110));
	...
	// sjyi 2024.02.28 헤어핀 외경(짝수열) 패턴 반전 기능 사용여부 추가
	m_hp.bReversal = hp.bReversal;

	// sjyi 2024.05.25 Sortie 반복, 스파이럴 패턴, 모래시계 패턴 추가
	m_hp.nGenDir = hp.nGenDir;

	m_hp.bEnableSortiRpt = hp.bEnableSortiRpt;
	m_hp.nSortieCnt = hp.nSortieCnt;
	m_hp.nSortieDir = hp.nSortieDir;

	m_hp.HpSize2nd2 = hp.HpSize2nd2;

	m_hp.bUseOutToInP1 = hp.bUseOutToInP1;
	m_hp.bUseOutToInP2 = hp.bUseOutToInP2;
	m_hp.bCloseSpiralP1 = hp.bCloseSpiralP1;
	m_hp.bCloseSpiralP2 = hp.bCloseSpiralP2;

	for (int i = 0; i < MAX_NUM_REPEAT_WELD; i++)
	{
		m_hp.m_nSortieRptMap[i] = hp.nSortieCnt;
	}
}
...
//#define LASER_MAX_POWER 5000.0
	...
	//double dTmpPower = dPower * m_pDoc->m_config.m_nLaserMax;
...
{	
	...
	// Laser Emission ON/OFF 확인
	_tracer(TRACE_NORMAL, 9999, L"DoVScan, GetDoEmission = [%d]", m_pScanner->GetDOEmission());
	if (!m_pScanner->GetDO(m_pScanner->GetDOEmission())) // Emission OFF 이면
	{	
		// Power ON Bit ON
...
UINT CIWSWorkHairpin::StartVScanThread(LPVOID pParam)
{
	...
	// Thread Run Flag TRUE로 변경
	pMe->m_bRunVScan = TRUE;

	// 워크 모드에 따라 실행되는 함수 다름.
	switch (pMe->m_iWorkMode)
	{
	case WORK_MODE_VSCAN:	// 스캔모드
		pMe->StartVScanThreadBody();
		break;
	case WORK_MODE_INSPECTION:	// 검사모드??
...
// Vision Scan 실제 실행 함수
UINT CIWSWorkHairpin::StartVScanThreadBody()
{	
	// 대기화면 다이얼로그 설정 
	m_pDlgWait->AssignStopFlag(NULL);		// Stop Button 비활성
	m_pDlgWait->SetMessage(L"Image Scanning...."); // 텍스트 메세지 설정
	m_pDlgWait->CenterWindow();				// 화면 중심에 표시
	m_pDlgWait->ShowWindow(SW_SHOW);		// 화면 보이기

	// 스티치(모자이크 이미지 합성) 완료 Flag  FALSE로 설정
	...
	// 함수 이름은 스캔 하고 저장, 
	// 하지만 실제 하는 내용은 또 다른 Vision Scan Thread 시작
	g_pGrabImgDlg->ScanAndSave(strStitchImg, TRUE, &m_nRefImgPixW, &m_nRefImgPixH, &m_fRefImgSizeW, &m_fRefImgSizeH);

	// 아래 while문은 단순 완료 Flag만 확인
	// 쓰레드를 이중으로 만든 이유는???
	while (m_bRunVScan) // 쓰레드 Run Flag 확인
	{
		// 스티치(모자이크 이미지 합성) 완료 Flag 확인, TRUE일 경우, 쓰레드 종료
	...
	//if (m_pDoc->m_work.m_b2DInspection == TRUE) // ehjang 2D Insp. 추가
	if (m_pDoc->GetUseInterface() == TRUE) // Online일 때만 스캔 후,, 바로 2D 검사.(Offline일 때는 Vision Scan만))
	{
		if (m_pDoc->GetDo2DInspection() == TRUE)
		{
			if (m_pDoc->m_work.m_b2DInspection == TRUE && m_pDoc->m_plc.GetReWeld() == FALSE) // ehjang 2D Insp. 추가, Rework 일때도 들어오지 마라
			{
				// sjyi 2023.07.14 2D검사 결과 반영을 위해 수정
				m_pDoc->Run2DInspection(FALSE); // ehjang 2D Insp. 추가

				// 2D 결과를 가져온다...
				CString strResult(_T(""));
				long okCount(0);
				long ngCount(0);
				m_pDoc->m_pHairpin2D->GetTotalResult(strResult, okCount, ngCount);

				// NG가 있을 경우 2D 검사 결과창을 띄운다.
				BOOL b2DInspectOK = TRUE;
				m_pDoc->m_pHairpin2D->ResultViewerDoModal();
				...
				if (b2DInspectOK)
				{
					...
					//m_pDoc->m_plc.Set2DInspectResult(1); // OK
					m_pDoc->Set2DInspectionResult(1);
				}
				else
				{
					// sjyi 2023.07.16 2D검사 결과 확인 함수 추가(to PLC)
					//m_pDoc->m_plc.Set2DInspectResult(2); // NG
					m_pDoc->Set2DInspectionResult(2);
				}
			}
			else
			{
				_tracer(TRACE_ERROR, 999, L"Recipe is Not 2D Inspection Mode");
				m_pDoc->Set2DInspectionResult(2); // NG
			}
		}

		// sjyi 2024.05.28 여기 참고하여 수정 WeldComplete 수정,
		// sjyi 2024.05.28 (추가 코멘트) 아래 내용 MainFrame 의 OnUwmCIWSwork함수로 이동
		// 항상 스캔하는 모드이고, PLC에서 검사 요청 신호가 들어 왔으면, 스캔 완료하고 Response 신호 ON
		// sjyi 2023.07.18 m_bUseAlwasScan 사용 안함(사유, PLC에서 2D모드-스캔모드 활성화)
		//if (m_pDoc->m_config.m_bUseAlwaysScan && m_pDoc->m_plc.GetReqInspect())
		//if (m_pDoc->GetDo2DInspection() && m_pDoc->m_plc.GetReqInspect())
		//{
		//	m_pDoc->m_plc.SetInspectReq(TRUE);
		//}
		////else if (m_pDoc->m_config.m_bUseAlwaysScan && m_pDoc->m_plc.GetReqInspect())
		//else if (m_pDoc->GetAllwaysScan() && m_pDoc->m_plc.GetReqInspect())
		//{
		//	m_pDoc->m_plc.SetInspectReq(TRUE);
		//}
	}
	...
	// Vision Scan 완료되어 Main Window로 Event 발생
	if (m_hRcv)
		::PostMessage(m_hRcv, UWM_CIWSWORK, EVENT_WORK_VSCAN, 0);

	// 쓰레드 Run Flag FALSE로 설정
	m_bRunVScan = FALSE;
...
UINT CIWSWorkHairpin::StartInspectionBody()
{
	...
	// While문 일부 변경
	// 변경 사유, m_bRunVScan이 IsDonStitchImage(스캔이미지 생성) 보다 빨리 떨어지는 경우 있음.
	if (m_bRunVScan)
	{
		while (1)
		{
...
void CIWSWorkHairpin::ModifyHairpin(int n1st, int n2nd, BOOL bUpdate, double dRotateAngle, double dRotateAngle2, BOOL bKeepRotation, BOOL bReversal, BOOL bSlotChange)
{
	// 0 = All, 1~4 = Layer
	...
	for (auto group : (*pJobGroup))
	{
		for (int i = 0; i < group->m_jobs.size(); i++)
		{
			if (bSlotChange == FALSE) // Layer Change
			{
				//nLayer = (group->m_nIndex % (m_nDefLayer / 2)) + 1;
				nLayer = group->m_nLayer;
				if (i < 2 && n1st != 0)		// 1st pattern
				{
					if (nLayer != n1st || n1st == 6)
					{
						idx++;
						continue;
					}
				...
				else if (i >= 2 && n2nd != 0)// 2nd pattern
				{
					if (nLayer != n2nd || n2nd == 6)
					{
						idx++;
						continue;
					}
			...
			{
				nSlotNo = group->m_nSlot + 1;
				if (i < 2 && nSlotNo != n1st)		// 1st pattern
				{
					idx++;
					continue;
				}
				else if (i >= 2 && nSlotNo != n2nd)// 2nd pattern
				{
					idx++;
					continue;
				}
...
//			if (!((idx + 1) % MAX_NUM_GRP)) // 2nd pattern
			...
			if (i >= 2) // 2nd pattern
			{
				if (bUpdate)
				{
					...
					// sjyi 2022-08-29 모래시계 패턴을 위해 변수 추가
					jobWeld->m_d2NdWidth2 = m_hp.HpSize2nd2;

					// sjyi 2022-09-15 Shape 변수 추가
					jobWeld->m_nShape1st = -1;
					jobWeld->m_nShape2nd = m_hp.nWeldShapeP2;
				}
				...
				case IWS_2ND_LINE: // Line
				{
					...
					nx = cx / mag;
					ny = cy / mag;
					handle = lcBlockAddLine(hBlock, (cx + nx * (HpSize + ol))
						, (cy + ny * (HpSize + ol))
						, (cx - nx * (HpSize + ol))
						, (cy - ny * (HpSize + ol)));
				} break;
				case IWS_2ND_ZIGZAG: // Zigzag
				{
				...
				case IWS_2ND_ZIGZAG2: // Zigzag2
				{
					...
					handle = GenZigZagPatrn(hBlock, cx + hp_cx, cy + hp_cy, theta, HpSize, HpSizeW, m_hp.nWeldZigZagFreqP2);
				...
				{
					entity = new CEntyPolyline(TRUE);
					double HpLength = 0;
					double HpSizeW = jobWeld->m_d2NdWidth;
					double HpSizeW2 = jobWeld->m_d2NdWidth2;
					...
					tmpx = (points[idx - 1].x - points[idx - 2].x);
					tmpy = (points[idx - 1].y - points[idx - 2].y);
					theta = atan2(tmpy, tmpx);			  // 1차패턴 두 중심점을 연결했을 때, 각도
					double dTheta2 = 90.0 * M_PI / 180.0; // 기본 회전각(2차패턴은 1차 패턴대비 90도 회전되어 있음.)

					// 길이 계산
					HpLength = _hypot(tmpx, tmpy) * 0.5 + jobWeld->m_d2NdOver;

					/////////////////////////////////////////////////////////////////////
					// 모래시계 회전 관련 
					/////////////////////////////////////////////////////////////////////
					double dTheta_bottom, dTheta_Top;		// 헤어핀 원점기준, 1차패턴의 회전각(Bottom - 1-1차, Top - 1-2차)
					double dTheta_Center, dTheta_CenterRAD;	// 헤어핀 원점기준, 1차패턴 두 지점의 중심에 대한 회전각(중심각도)
					//double dTheta_Entity, dTheta_EntityRAD; // Entity 자체가 기울어진 각도
					double dHp_cx, dHp_cy;

					// 헤어핀 Center 획득
					GetHairpinCenter(dHp_cx, dHp_cy);

					// Bottom위치 각도 계산(헤어핀 중심 대비)
					dTheta_bottom = atan2((points[idx - 2].y - dHp_cy), (points[idx - 2].x - dHp_cx)) * LC_RAD_TO_DEG;

					// Top위치 각도 계산(헤어핀 중심 대비)
					dTheta_Top = atan2((points[idx - 1].y - dHp_cy), (points[idx - 1].x - dHp_cx)) * LC_RAD_TO_DEG;

					// 평균 회전각 계산(1-1차, 1-2차 평균 회전각 - 중심점의 회전각)
					dTheta_Center = (dTheta_Top + dTheta_bottom) / 2;
					dTheta_CenterRAD = dTheta_Center * LC_DEG_TO_RAD;

					//// Entity 회전각 계산(평균회전각, 즉 줌심점의 회전각 대비 회전각도)
					//dTheta_Entity = theta * LC_RAD_TO_DEG - dTheta_Center;
					//dTheta_EntityRAD = dTheta_Entity * LC_DEG_TO_RAD;

					handle = GenSandGlassPatrn(hBlock,
						cx + hp_cx,
						cy + hp_cy,
						HpLength,
						HpLength,
				...
				case IWS_2ND_SPIRAL: // sjyi 2024.02.01 2차 패턴에 Spiral 추가
				{
					entity = new CEntitySpiral;
					cx = double(points[idx - 2].x + points[idx - 1].x) * 0.5;
					cy = double(points[idx - 2].y + points[idx - 1].y) * 0.5;
					...
					//r1 = _hypot(tmpx, tmpy) / 2;
					//r2 = m_hp.HpSize2nd * 0.5;
					r1 = jobWeld->m_d2NdWidth * 0.5;
					r2 = jobWeld->m_d2NdWidth2 * 0.5;

					handle = lcBlockAddSpiral(hBlock, cx, cy, r1, m_hp.nWeldZigZagFreqP2, TRUE, m_hp.bCloseSpiralP2);
					lcPropPutFloat(handle, LC_PROP_SPIR_R2, r2);

					((CEntitySpiral*)entity)->m_dirOut2In = m_hp.bUseOutToInP2;
			...
			else // 1st pattern
			{
				if (bUpdate)
				{
					...
					// sjyi 2022-09-15 Shape 변수 추가
					jobWeld->m_nShape1st = m_hp.nWeldShapeP1;
					jobWeld->m_nShape2nd = -1;
				}
				...
				switch (m_hp.nWeldShapeP1)
				{
					case IWS_1ST_ELLIPSE:
					{
						...
						theta = atan2(cy, cx) + M_PI / 2;
						handle = lcBlockAddEllipse(hBlock, cx + hp_cx, cy + hp_cy, r1, r2, theta, 0, 0);
						...
						if (bKeepRotation == TRUE) // 기존 헤어핀 회전 정보 그대로 유지
						{
							...
							if (bReversal){
								if (i % 2) { // 외경만 회전
									dRotateAngleRAD = (jobWeld->GetRotation() + 180) * LC_DEG_TO_RAD;
								}
							}
					...
					case IWS_1ST_ZIGZAG:
					{
						...
						cy = points[idx].y - hp_cy;
						theta = atan2(cy, cx) + M_PI / 2;
						handle = GenZigZagPatrn(hBlock, cx + hp_cx, cy + hp_cy, theta, HpSize, HpSizeW, m_hp.nWeldZigZagFreqP1);
					...
					break;
					case IWS_1ST_SPIRAL: // sjyi 2024.01.01 1차패턴 스파이럴 추가
					{
						entity = new CEntitySpiral;
						cx = points[idx].x - hp_cx;
						cy = points[idx].y - hp_cy;
						...
						r2 = jobWeld->m_d1StWidth * 0.5;
						theta = atan2(cy, cx) + M_PI / 2;
						handle = lcBlockAddSpiral(hBlock, cx + hp_cx, cy + hp_cy, r1, m_hp.nWeldZigZagFreqP1, TRUE, m_hp.bCloseSpiralP1);
						lcPropPutFloat(handle, LC_PROP_SPIR_R2, r2);
						((CEntitySpiral*)entity)->m_dirOut2In = m_hp.bUseOutToInP1;

						//ksu 2023.11.21 1차 패턴 회전 병합
						if (bKeepRotation == TRUE) // 기존 헤어핀 회전 정보 그대로 유지
						{
							dRotateAngleRAD = jobWeld->GetRotation() * LC_DEG_TO_RAD;
							lcEntRotate(handle, cx + hp_cx, cy + hp_cy, dRotateAngleRAD);
			...
			// sjyi 2025.01.26 용접 가능여부 다시 설정(펜 재설정을 위해 추가)
			jobWeld->m_nOriPenID = jobWeld->m_iPenID;
			jobWeld->SetDoJob(jobWeld->IsDoJob());
...
void CIWSWorkHairpin::AlignHairpin(HANDLE handle)
{
	...
	if (m_pDoc->m_config.m_bUseHairpin == FALSE)
	{
		return;
	}
	...
	// sjyi 2024.02.17 추가, 헤어핀 패턴 회전 각도 추가
	double dRotateAngleRAD = 0;
	BOOL bOctagon = FALSE;
...
//			if (!((idx + 1) % MAX_NUM_GRP)) 
			...
			if (nMaxNumGrp == 3) // sjyi 2nd 패턴이 zigzag2가 아님 기존과 동일
			{
				if (!((idx + 1) % nMaxNumGrp))
				{
					...
					{
						case LC_ENT_ELLIPSE: 
							AlignEntityEllipse2nd(Ent, points, idx, &((CJobWeld*)p)->m_iPenID, ((CJobWeld*)p)->m_d2NdOver);
							((CJobWeld*)p)->m_nShape2nd = IWS_2ND_ELLIPSE;
							break;
						case LC_ENT_LINE: 
							AlignEntityLine2nd(Ent, points, idx, &((CJobWeld*)p)->m_iPenID, ((CJobWeld*)p)->m_d2NdOver); 
							((CJobWeld*)p)->m_nShape2nd = IWS_2ND_LINE;
							break;
						case LC_ENT_POLYLINE: 
							AlignEntityZigZag2nd(Ent, points, idx, &((CJobWeld*)p)->m_iPenID, ((CJobWeld*)p)->m_d2NdWidth, ((CJobWeld*)p)->m_d2NdOver); 
							((CJobWeld*)p)->m_nShape2nd = IWS_2ND_ZIGZAG;
							break;
						case (LC_ENT_POLYLINE + LC_ENT_CUSTOM):	//2차 패턴이 모래시계 모양
							AlignEntitySandGlass(Ent, points, idx, &((CJobWeld*)p)->m_iPenID, ((CJobWeld*)p)->m_d2NdWidth, ((CJobWeld*)p)->m_d2NdOver, ((CJobWeld*)p)->m_d2NdWidth2);
							((CJobWeld *)p)->m_nShape2nd = IWS_2ND_SANDGLASS;
							break;
						case LC_ENT_SPIRAL: // sjyi 2024.02.01 Spiral 패턴 추가
							AlignEntitySprial2nd(Ent, points, idx, &((CJobWeld*)p)->m_iPenID);
							((CJobWeld *)p)->m_nShape2nd = IWS_2ND_SPIRAL;
							break;
					}
				}
				else
				{ 
					...
					{
						case LC_ENT_ELLIPSE: 
							AlignEntityEllipse1st(Ent, &((CJobWeld*)p)->m_iPenID, ((CJobWeld*)p)->GetRotation());
							((CJobWeld*)p)->m_nShape1st = IWS_1ST_ELLIPSE;
							//RotateHairpin(Ent, ((CJobWeld*)p)->GetRotation());
							break;
						case LC_ENT_LINE: 
							AlignEntityLine1st(Ent, &((CJobWeld*)p)->m_iPenID, ((CJobWeld*)p)->GetRotation());
							((CJobWeld*)p)->m_nShape1st = IWS_1ST_LINE;
							//RotateHairpin(Ent, ((CJobWeld*)p)->GetRotation());
							break;
						case LC_ENT_POLYLINE: 
						{
							bOctagon = AlignEntityZigZag1st(Ent, &((CJobWeld*)p)->m_iPenID, ((CJobWeld*)p)->GetRotation());
							if (bOctagon) {
								((CJobWeld*)p)->m_nShape1st = IWS_1ST_OCTAGON;
								//RotateHairpin(Ent, ((CJobWeld*)p)->GetRotation());
							}
							else {
								((CJobWeld*)p)->m_nShape1st = IWS_1ST_ZIGZAG;
							}
						}
						break;
						case LC_ENT_SPIRAL: // sjyi 2024.02.01 Spiral 추가
							AlignEntitySpiral1st(Ent, &((CJobWeld*)p)->m_iPenID, ((CJobWeld*)p)->GetRotation());
							((CJobWeld*)p)->m_nShape1st = IWS_1ST_SPIRAL;
							//RotateHairpin(Ent, ((CJobWeld*)p)->GetRotation());
							break;
					}
				}
			...
			else  // sjyi 2nd 패턴이 ZigZag2
			{
				...
				if (nSubIndexCheck >= 2 ) // 2nd
				{
					...
					CEntity *Ent = p->GetEntity();
					AlignEntityZigZag2_2nd(Ent1st, Ent, &((CJobWeld*)p)->m_iPenID, ((CJobWeld*)p)->GetRotation());
					((CJobWeld*)p)->m_nShape2nd = IWS_2ND_ZIGZAG2;

					// sjyi 2024.02.17 Rotation 추가 수정
					//RotateHairpin(Ent, ((CJobWeld*)p)->GetRotation());
				}
				else
				{ 
...
void CIWSWorkHairpin::AlignEntityEllipse1st(CEntity *pEnt, int *penID, double dRotationAngle)
{
	CEntyEllipse *e = (CEntyEllipse*)pEnt;
	double hp_cx, hp_cy;
	GetHairpinCenter(hp_cx, hp_cy);
	e->SetPen(penID);	// hslee
	e->RotAngle = atan2(e->center.y - hp_cy, e->center.x - hp_cx) * LC_RAD_TO_DEG + 90.0f;
	e->RotAngle += dRotationAngle;
	e->UpdateEntity(NULL, COleVariant::COleVariant(0.0));
}

void CIWSWorkHairpin::AlignEntityLine1st(CEntity *pEnt, int *penID, double dRotationAngle)
{
	CEntyLine *e = (CEntyLine*)pEnt;
	double hp_cx, hp_cy;
	GetHairpinCenter(hp_cx, hp_cy);
	...
	double x = e->m_cen.x - e->m_offset.x - hp_cx;	// 중심점 획득(x)
	double y = e->m_cen.y - e->m_offset.y - hp_cy;	// 중심점 획득(y)
	//double mag = sqrt(x * x + y * y);		// 중심으로부터의 거리

	//double nx = -y / mag;
	//double ny = x / mag;

	Point3Dbl d = e->point0 - e->point1;
	double len = _hypot(d.x, d.y); // 선의 길이 계산
	...
	// 센터점의 각도 계산
	double dCenterAngRAD = atan2(y, x);
	double dEntityAng = dCenterAngRAD * LC_RAD_TO_DEG;

	// 기본 회전(+90 원점 기준 법선이어야 하므로)
	dEntityAng += 90;

	// 추가 회전(각 헤어핀의 회전 각도)
	dEntityAng += dRotationAngle;
	...
	// 각도 변환(degree to radian)
	double dRatationAngleRAD = dEntityAng * LC_DEG_TO_RAD;

	// 원점 계산 -------------------------->
	Point3Dbl ptStartOrg, ptEndOrg;
	// 시작점 (-0.5l, 0) 
	ptStartOrg.x = -0.5*len;
	ptStartOrg.y = 0;
	...
	// 원점에서 회전 적용 --------------------------->
	Point3Dbl ptStart, ptEnd;
	ptStart.x = cos(dRatationAngleRAD) * ptStartOrg.x;
	ptStart.y = sin(dRatationAngleRAD) * ptStartOrg.x;
	
	ptEnd.x = cos(dRatationAngleRAD) * ptEndOrg.x;
	ptEnd.y = sin(dRatationAngleRAD) * ptEndOrg.x;
	// 원점에서 회전 적용 <---------------------------

	// 포인트 이동
	e->point0.x = x + ptStart.x;	// 시작점(x) = 중심점(x) + 원점기준 회전 시작점(x)
	e->point0.y = y + ptStart.y;	// 시작점(y) = 중심점(y) + 원점기준 회전 시작점(y)
	e->point1.x = x + ptEnd.x;		// 끝점(x) = 중심점(x) + 원점기준 회전 끝점(x)
	e->point1.y = y + ptEnd.y;		// 끝점(y) = 중심점(y) + 원점기준 회전 끝점(y)
...
BOOL CIWSWorkHairpin::AlignEntityZigZag1st(CEntity *pEnt, int *penID, double dRotateAngle)
{
	BOOL bOctagon = FALSE;
	...
	// 포인트 좌표 확인(시작좌표와 끝좌표가 같으면 Octagon)
	if ((e->m_points[nSize - 1].x == e->m_points[0].x) &&
		(e->m_points[nSize - 1].y == e->m_points[0].y))
	{
		bOctagon = TRUE;
	}
	...
	lcEntRotate(e->m_hCadEntity, x + e->m_offset.x + hp_cx, y + e->m_offset.y + hp_cy, theta*LC_DEG_TO_RAD );
	//TRACE("ang current (%f), target(%f), del = %f, (%.3f, %.3f)\n", angCur, angTarg, angTarg - angCur, x, y);
...
void CIWSWorkHairpin::CalcHairpinInfo()
{
	...
	PointFloat *points = new PointFloat[nSlots];  // Layer 蹂濡 CircleFitting???ъ??Sample Points
	if (m_pFitCircle) delete[] m_pFitCircle;
...
void CIWSWorkHairpin::ModifyHairpinWeldRepeat(int nP1, int nP2, int n1st, int n2nd)
{
...
void CIWSWorkHairpin::GenHairPin(int nSlot, int nLayer, double startAng,
	...
	double HpSize2nd_2, int nGenDir)
{
	double cx, cy;
	...
	for (int i = 0; i < nSlot; i++)
	{
		...
		for (int j = 0; j < nLayer; j++)
		{
			...
			case IWS_1ST_OCTAGON:
				entity = new CEntyPolyline;
				break;
			case IWS_1ST_SPIRAL: // Sprial, sjyi 2024.02.01 추가
				entity = new CEntitySpiral;
				break;
			default:
				entity = new CEntyEllipse;
			...
			case IWS_1ST_ZIGZAG:
				handle = GenZigZagPatrn(hBlock
					, cx + hp_cx
					, cy + hp_cy
					, theta + M_PI / 2, HpSize, HpSizeW, m_hp.nWeldZigZagFreqP1);
				break;
			case IWS_1ST_OCTAGON:		// Octagon
				handle = GenPolygon(hBlock, cx + hp_cx, cy + hp_cy, HpSize, HpSizeW, theta + M_PI / 2);
				break;
			case IWS_1ST_SPIRAL: // Spiral, sjyi 2024.02.01 추가
				handle = lcBlockAddSpiral(hBlock, cx + hp_cx, cy + hp_cy, r1, m_hp.nWeldZigZagFreqP1, TRUE, m_hp.bCloseSpiralP1);
				lcPropPutFloat(handle, LC_PROP_SPIR_R2, r2);
				((CEntitySpiral*)entity)->m_dirOut2In = m_hp.bUseOutToInP1;
				entity->m_cen.x = cx + hp_cx;
				entity->m_cen.y = cy + hp_cy;
				break;
			default:
			...
			if (m_hp.nWeldShapeP2 == IWS_2ND_ZIGZAG2) // Zigzag2
			// sjyi 2022-05-23 수정<--
			{
				...
				handle = GenZigZagPatrn(hBlock
					, cx + hp_cx
					, cy + hp_cy
					, theta + M_PI / 2, dOverLen, HpSize2nd, m_hp.nWeldZigZagFreqP2);
			...
			else
			{
				...
				if (!((j + 1) % 2))
				{
					...
					switch (m_hp.nWeldShapeP2)
					{
					case IWS_2ND_ELLIPSE:	entity = new CEntyEllipse;	break;
					case IWS_2ND_LINE:		entity = new CEntyLine;		break;
					case IWS_2ND_ZIGZAG:	entity = new CEntyPolyline; break;
					case IWS_2ND_SANDGLASS: entity = new CEntyPolyline(TRUE); break;
					case IWS_2ND_SPIRAL:	entity = new CEntitySpiral; break; // sjyi 2024.02.01 2차패턴에 Sprial 추가
					...
					pJob12->m_d2NdWidth2 = HpSize2nd_2;

					// 반경 계산
					// radius : 원점에서 Layer1과 Layer2 사이의 중심에 대한 거리의 1/2 -- 의미 파악 필요.
					// r1 : 1차패턴 2개 사이의 거리를 지름으로 했을 때, 반지름(길이)
					// r2 : 파라미터에 설정된 길이를 지름으로 했을 때, 반지름(길이)
					radius = (pLayerDia[j - 1] + pLayerDia[j]) * 0.25;
					r1 = (pLayerDia[j] - pLayerDia[j - 1])*0.25;
					r2 = HpSize2nd * 0.5;
					...
					{
					case IWS_2ND_ELLIPSE:	// Ellipse
						handle = lcBlockAddEllipse(hBlock, cx + hp_cx, cy + hp_cy, r1 + ol, r2, theta, 0, 0);
						break;
					case IWS_2ND_LINE: // line
					...
					case IWS_2ND_SANDGLASS:
					{
						double dTheta2 = 90.0 * M_PI / 180.0;
						handle = GenSandGlassPatrn(hBlock,
							cx + hp_cx, // 중심점 x
							cy + hp_cy, // 중심점 y
							r1 + ol,	// 길이1 - 모래시계 위쪽 길이
							r1 + ol,	// 길이2 - 모래시계 아래쪽 길이
							HpSize2nd,		// 폭1 - 모래시계 위,아래면 폭(넓은면 폭)
							HpSize2nd_2,	// 폭2 - 모래시계 짧은 폭(오목한 폭)
							theta,		// 회전각(헤어핀 원점 기준)
							thetaCenter);	// 중심점의 회전각(헤어핀 원점 기준)
					}
					break;
					case IWS_2ND_SPIRAL: // sjyi 2024.02.01 2차 패턴에 SPIRAL 추가
						handle = lcBlockAddSpiral(hBlock, cx + hp_cx, cy + hp_cy, HpSize2nd * 0.5, m_hp.nWeldZigZagFreqP2, TRUE, m_hp.bCloseSpiralP2);
						lcPropPutFloat(handle, LC_PROP_SPIR_R2, HpSize2nd_2 * 0.5);
						((CEntitySpiral*)entity)->m_dirOut2In = m_hp.bUseOutToInP2;
						break;
...
void CIWSWorkHairpin::AlignEntityZigZag2_2nd(CEntity* pEnt1st, CEntity *pEnt, int *penID, double dRotateAngle)
{
	...
	lcEntRotate(e->m_hCadEntity, x + e1st->m_offset.x + hp_cx, y + e1st->m_offset.y + hp_cy, theta*LC_DEG_TO_RAD);
	lcEntUpdate(e->m_hCadEntity);
	e->ChangedEntity(e->m_hCadEntity);
...
void CIWSWorkHairpin::SetDivParameters(int &nWeldType, int &nJumpOffset, int &nStartSlot)
{
	switch (nWeldType)
	{
	case IWS_JOB_DIV_INOUT:
		nWeldType = IWS_JOB_DIV_INOUT;
		nStartSlot = 1;
		nJumpOffset = m_hp.nDrawSlot / 3;
		break;
	case IWS_JOB_DIV_OUTIN:
		nWeldType = IWS_JOB_DIV_OUTIN;
		nStartSlot = 1;
		nJumpOffset = m_hp.nDrawSlot / 3;
		break;
	...
	case IWS_JOB_CRING:
		...
		nJumpOffset = 1;
		break;
	case IWS_JOB_CRING_DIVISION_BLOW:
		nWeldType = IWS_JOB_DIV_INOUT;
		nStartSlot = 1;
		nJumpOffset = 1;
		break;
...
HANDLE CIWSWorkHairpin::GenSandGlassPatrn(HANDLE hBlock, double cx, double cy, double dLength1, double dLength2, double dWidth1, double dWidth2, double dTheta, double dThetaCenter)
{
	// double cx;			// 패턴의 중심점(x)
	// double cy;			// 패턴의 중심점(y)
	// double dLength1;		// 모래시계 중심에서 위쪽의 길이
	// double dLength2;		// 모래시계 중심에서 아래쪽의 길이
	// double dWidth1;		// 모래시계 넓은 폭
	// double dWidth2;		// 모래시계 짧은 폭(오목한 쪽의 폭)
	// double dTheta;		// 패턴이 기울어진 각도(0도 기준)
	// double dThetaCenter; // 헤어핀 센터에서 패턴 중심점까지의 각도

	// Draw 영역에 PolyLine Entity 생성
	HANDLE hPline = lcBlockAddPolyline(hBlock, 0, FALSE, FALSE);

	// 기본 회전각 계산(Radian)
	double dTheta2 = 90 * LC_DEG_TO_RAD; // 2차패턴 기본 회전각(1차패턴 대비 90도 회전)

	// double dThetaEntity; // 실제 패턴이 기울어진 각도(헤어핀 중심점 각도 기준)
	double dThetaEntity = (dTheta* LC_RAD_TO_DEG - dThetaCenter * LC_RAD_TO_DEG);
	dThetaEntity = dThetaEntity * LC_DEG_TO_RAD;

	// 폭의 1/2 계산(x축 좌표로 활용)
	double dW1_50 = dWidth1 * 0.5;		// dWidth1 * 1/2
	double dW2_50 = dWidth2 * 0.5;		// dWidth2 * 1/2

	// PolyLine 패턴 생성
	// SandGlass(모래시계) -- 현재, Length1, Length2 동일
	//      0        5   --------------
	//                          ↑
	//                       Length1(dLengthNew1)
	//							↓
	...
	double dOffsetX = dLength1 * sin(dThetaEntity);
	double dLengthNew1 = dLength1 * cos(dThetaEntity);
	double dLengthNew2 = dLength2 * cos(dThetaEntity);

	lcPlineAddVer(hPline, 0, -dW1_50 - dOffsetX, dLengthNew1);	//0
	lcPlineAddVer(hPline, 0, -dW2_50, 0);						//1
	lcPlineAddVer(hPline, 0, -dW1_50 + dOffsetX, -dLengthNew2);	//2
	lcPlineAddVer(hPline, 0, dW1_50 + dOffsetX, -dLengthNew2);	//3
	lcPlineAddVer(hPline, 0, dW2_50, 0);						//4
	lcPlineAddVer(hPline, 0, dW1_50 - dOffsetX, dLengthNew1);	//5
	lcPlineAddVer(hPline, 0, -dW1_50 - dOffsetX, dLengthNew1);	//0

	lcPlineEnd(hPline);

	lcEntRotate(hPline, 0, 0, dTheta2);			// 기본 회전(2차패턴은 90도 회전되어야 함)
	lcEntRotate(hPline, 0, 0, dThetaCenter);	// 원점 중심으로 회전

	lcEntMove(hPline, cx, cy);			// 중심좌표 이동
	lcEntUpdate(hPline);

	return hPline;
}

void CIWSWorkHairpin::AlignEntitySandGlass(CEntity *pEnt, POINTF* points, int idx, int *penID, double d2ndWidth, double dOverLen, double d2ndWidth2)
{
	// Event 비활성화
	//SetCadEventEnable(FALSE);
	...
	// 1차패턴의 두점의 좌표 획득 P0(x0, y0), P1(x1, y1)
	double x0 = (double)points[idx - 1].x;
	double y0 = (double)points[idx - 1].y;
	double x1 = (double)points[idx - 2].x;
	double y1 = (double)points[idx - 2].y;

	// 패턴의 폭, Over Length 획득
	double wid = d2ndWidth;			// 폭
	double ol = dOverLen;			// Over Length
	double wid2 = d2ndWidth2;		// 폭2(안쪽 폭)
	double dLength1 = _hypot(x0 - x1, y0 - y1) * 0.5 + ol; // 모래시계 길이(위쪽)
	double dLength2 = dLength1;		// 모래시계 길이(아래쪽)

	// 회전 각도 계산
	double dTheta = atan2(y, x);			// 1차 패턴 두 점을 연결했을 때, 각도(0도 기준)
	double dTheta2 = 90.0 * M_PI / 180.0;	// Offset 회전(2차 패턴의 경우, 90도 회전을 한번더 해야 함.

	// 폭 1/2 계산(좌표로 활용)
	double dW1_50 = wid * 0.5;		// dWidth1 * 1/2
	double dW2_50 = wid2 * 0.5;		// dWidth2 * 1/2
	...
	// 모래시계 회전 관련 
	/////////////////////////////////////////////////////////////////////
	double dTheta_bottom, dTheta_Top;		// 1차패턴의 회전각(Bottom - 1-1차, Top - 1-2차)
	double dTheta_Center, dTheta_CenterRAD;	// 1차패턴 두 지점의 중심에 대한 회전각(중심각도)
	double dTheta_Entity, dTheta_EntityRAD; // Entity 자체가 기울어진 각도 (중심각도 대비 기울어진 각도)
	double dHp_cx, dHp_cy;
	...
	// Entity 회전각 계산(평균회전각-중심회전각 대비 회전각도)
	dTheta_Entity = dTheta * LC_RAD_TO_DEG - dTheta_Center;
	dTheta_EntityRAD = dTheta_Entity * LC_DEG_TO_RAD;
	...
	//TRACE(_T("[SJYI Test] Bottom(%.3f, %.3f), Top(%.3f, %.3f) Angle(%.3f, %.3f, %.3f) Theta(%.3f)\n"),
	//	points[idx - 2].x, points[idx - 2].y, 
	//	points[idx - 1].x, points[idx - 1].y, 
	//	dTheta_bottom, dTheta_Top, dTheta_Center,
	//	dTheta * LC_RAD_TO_DEG);

	double dOffsetX = dLength1 * sin(dTheta_EntityRAD);
	double dLengthNew = dLength1 * cos(dTheta_EntityRAD);


	lcPlineAddVer(hPline, 0, -dW1_50 - dOffsetX, dLengthNew);	//0
	lcPlineAddVer(hPline, 0, -dW2_50, 0);						//1
	lcPlineAddVer(hPline, 0, -dW1_50 + dOffsetX, -dLengthNew);	//2
	lcPlineAddVer(hPline, 0, dW1_50 + dOffsetX, -dLengthNew);	//3
	lcPlineAddVer(hPline, 0, dW2_50, 0);						//4
	lcPlineAddVer(hPline, 0, dW1_50 - dOffsetX, dLengthNew);	//5
	lcPlineAddVer(hPline, 0, -dW1_50 - dOffsetX, dLengthNew);	//0

	lcPlineEnd(hPline);

	lcEntRotate(hPline, 0, 0, dTheta2);				// 기본 회전(2차패턴은 90도 회전되어야 함)
	lcEntRotate(hPline, 0, 0, dTheta_CenterRAD);	// 원점 중심으로 회전
	...
	// 화면 Event 활성화 Flag ON
	//SetCadEventEnable(TRUE);

	// 변경된 Entity내용 반영(Path, Center 등)
	e->ChangedEntity(e->m_hCadEntity);
	e->UpdateEntity(NULL, COleVariant::COleVariant(0.0));
}

void CIWSWorkHairpin::AlignEntitySprial2nd(CEntity *pEnt, POINTF* points, int idx, int *penID)
{
	double tmpx, tmpy;
	double radius;
	double theta;
	...
	CEntitySpiral *e = (CEntitySpiral*)pEnt;
	e->SetPen(penID);	// hslee
	tmpx = (points[idx - 2].x + points[idx - 1].x) * 0.5 - hp_cx;
	tmpy = (points[idx - 2].y + points[idx - 1].y) * 0.5 - hp_cy;
	radius = _hypot(tmpx, tmpy);
	...
	e->m_rotAng = theta;
	e->m_center.x = cx + hp_cx;
	e->m_center.y = cy + hp_cy;
	e->m_cen.x = cx + hp_cx;
	e->m_cen.y = cy + hp_cy;
	e->m_offset.x = 0;
	e->m_offset.y = 0;
...
void CIWSWorkHairpin::AlignEntitySpiral1st(CEntity *pEnt, int *penID, double dRotationAngle)
{
	CEntitySpiral *e = (CEntitySpiral*)pEnt;
	double hp_cx, hp_cy;
	GetHairpinCenter(hp_cx, hp_cy);
	e->SetPen(penID);	// hslee
//	e->m_rotAng = atan2(e->m_center.y - hp_cy, e->m_center.x - hp_cx) * LC_RAD_TO_DEG + 90.0f;
	e->m_rotAng = atan2(e->m_center.y - hp_cy, e->m_center.x - hp_cx) * LC_RAD_TO_DEG + 90.0f;
	e->m_rotAng += dRotationAngle;

	e->UpdateEntity(NULL, COleVariant::COleVariant(0.0));
}

void CIWSWorkHairpin::SetCadEventEnable(BOOL bEnable)
{
	g_bEventEnable = bEnable;
}