...
BOOL CDlgVScanCalib::OnInitDialog()
{
	...
	m_pMergeCtrl = &(g_pGrabImgDlg->m_pScanImgsProc->m_pMergeCtrl);
	...
	float fXmm = (100.0f - m_iOverlap) * 0.01f* g_pGrabImgDlg->m_nGrabWd*resolX_umPcnt*0.001f;
	float fYmm = (100.0f - m_iOverlap) * 0.01f* g_pGrabImgDlg->m_nGrabHt*resolX_umPcnt*0.001f;

	m_iImgW = g_pGrabImgDlg->m_nGrabWd;
	m_iImgH = g_pGrabImgDlg->m_nGrabHt;
...
void CDlgVScanCalib::OnBnClickedRadioSave()
{
	...
	//g_pGrabImgDlg->m_pScanImgsProc->SetCalibData(m_sCalibPath, m_nBlockX, m_nBlockY);
	g_pGrabImgDlg->ChangeScanRange(resolX, resolY, m_dOldScanSx, m_dOldScanSy, m_dOldScanEx, m_dOldScanEy);
...
void CDlgVScanCalib::OnBnClickedOk()
{
	...
	g_pGrabImgDlg->m_pScanImgsProc->UseCalibration(m_pDoc->m_config.m_arCorrParam[iCurLensIdx].bUseCalib);
	CDialogEx::OnOK();
}
...
void CDlgVScanCalib::OnBnClickedButtonSaveData()
{
	...
	g_pGrabImgDlg->m_pScanImgsProc->SaveScanImageAll(strExePath);
}
