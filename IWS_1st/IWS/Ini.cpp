...
BOOL CIni::AppendString(LPCTSTR lpSection, LPCTSTR lpKey, LPCTSTR lpString) const
{
	...
	_stprintf_s(pNewString, _tcslen(psz) + _tcslen(lpString), _T("%s%s"), psz, lpString);
...
DWORD CIni::GetArray(LPCTSTR lpSection, LPCTSTR lpKey, LPTSTR lpBuffer, DWORD dwBufSize, LPCTSTR lpDelimiter, BOOL bTrimString) const
{
	...
	if (*psz != _T('\0'))
	{
		...
		{
			// just calculate the required buffer size
			const DWORD MAX_LEN = (const DWORD)(_tcslen(psz) + 2);
...
BOOL CIni::WriteInt(LPCTSTR lpSection, LPCTSTR lpKey, int nValue, int nBase) const
{
	...
	__IntToString(nValue, szValue, DEF_PROFILE_NUM_LEN, nBase);
...
{
	TCHAR szValue[DEF_PROFILE_NUM_LEN + 1] = _T("");
	__UIntToString(nValue, szValue, DEF_PROFILE_NUM_LEN, nBase);
	return WriteString(lpSection, lpKey, szValue);
}
...
BOOL CIni::WriteDataBlock(LPCTSTR lpSection, LPCTSTR lpKey, LPCVOID lpData, DWORD dwDataSize) const
{
	...
	LPTSTR psz = new TCHAR[dwDataSize * sizeof(TCHAR)*2 + 1];
	for (DWORD i = 0, j = 0; i < dwDataSize; i++, j += 2)
		_stprintf_s(&psz[j], sizeof(TCHAR)*2, _T("%02X"), lpb[i]);
...
DWORD CIni::__StringSplit(LPCTSTR lpString, LPTSTR lpBuffer, DWORD dwBufSize, LPCTSTR lpDelimiter, BOOL bTrimString)
{
	...
	*lpBuffer = _T('\0');
	...
	while (lpEnd != NULL)
	{
		...
		const DWORD SEG_LEN = (const DWORD)(_tcslen(pszSeg));
...
BOOL CIni::ParseDNTString(LPCTSTR lpString, SUBSTRPROC lpFnStrProc, LPVOID lpParam)
{
	...
	LPCTSTR p = lpString;
	DWORD dwLen = (const DWORD)(_tcslen(p));
...
void CIni::__IntToString(int nNumber, LPTSTR lpBuffer, DWORD dwBufSize, int nBase)
{
	switch (nBase)
	{
	case BASE_BINARY:
	case BASE_OCTAL:
	case BASE_HEXADECIMAL:
		__UIntToString((UINT)nNumber, lpBuffer, dwBufSize, nBase);
		break;

	default:
		_stprintf_s(lpBuffer, dwBufSize, _T("%d"), nNumber);
		break;
	}	
}

// Convert an unsigned integer into string representation, based on its base
void CIni::__UIntToString(UINT nNumber, LPTSTR lpBuffer, DWORD dwBufSize, int nBase)
{
	switch (nBase)
	{
	case BASE_BINARY:
	...
	case BASE_OCTAL:
		_stprintf_s(lpBuffer, dwBufSize, _T("%o"), nNumber);
		break;

	case BASE_HEXADECIMAL:
		_stprintf_s(lpBuffer, dwBufSize, _T("%X"), nNumber);
		break;

	default:
		_stprintf_s(lpBuffer, dwBufSize, _T("%u"), nNumber);
		break;
	}	
}
...
#pragma warning(pop)
	LPTSTR psz = new TCHAR[LEN + 1];
	_tcsncpy_s(psz, LEN, lpStart, LEN);
	psz[LEN] = _T('\0');
	return psz; // !!! Requires the caller to free this memory !!!
}
